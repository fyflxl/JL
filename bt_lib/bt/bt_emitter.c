...
#include "audio_enc.h"
#include "audio_enc/audio_sbc_codec.h"
...
u8 emitter_search_result(char *name, u8 name_len, u8 *addr, u32 dev_class, char rssi)
{
    ...
    /* #endif */
#if (RCSP_MODE)
    extern void rcsp_command_send_bt_scan_result(char *name, u8 name_len, u8 * addr, u32 dev_class, char rssi);
    rcsp_command_send_bt_scan_result(name, name_len, addr, dev_class, rssi);
#endif
...
extern cbuffer_t *iis_mic_cbuf;

struct bt_emitter_iis_dec_hdl {
    struct audio_stream *stream;	// 音频流
    struct pcm_decoder pcm_dec;		// pcm解码句柄
    struct audio_res_wait wait;		// 资源等待句柄
    struct audio_mixer_ch mix_ch;	// 叠加句柄
    ...
    u32 lost;
    u32 cbuf_use_max;
    u8 skip_cnt; //用来丢掉刚开iis时的前几包异常数据
    void *p_howling_hdl; //啸叫抑制句柄
};

static struct bt_emitter_iis_dec_hdl *bt_emitter_iis_dec = NULL;
...
static void adc_iis_output_handler(void *priv, s16 *data, int len)
{
    /* putchar('L'); */
    struct bt_emitter_mic_dec_hdl *dec = bt_emitter_iis_dec;
    s16 *outdat = NULL;
    int outlen = 0;
    int wlen = 0;
    int i = 0;
    /* printf("out_len=%d",len); */
#if 0 //这里替换成正弦波,测试使用,打开这里会把iis的数据替换成正弦波,用于测试
    int sin44k_frame_len = sizeof(sin44K) / 2;
    frame_bytes = read_44k_sine_data(data, len, frames_offset, 2);
    frames_offset += (frame_bytes >> 1) / 2;
    frames_offset = frames_offset % sin44k_frame_len;
#endif

    if (iis_mic_cbuf) {
        wlen = cbuf_write(iis_mic_cbuf, data, len);
        if (wlen != len) {
            putchar('w');
        }
        audio_decoder_resume(&dec->pcm_dec.decoder);
...
static int bt_emitter_iis_dec_read(void *hdl, void *data, int len)
{
    /* putchar('L'); */
    struct bt_emitter_iis_dec_hdl *dec = hdl;
    /* put_buf(data,len); */
    u32 r_len = cbuf_read(iis_mic_cbuf, data, len);
    if (r_len != len) {
        putchar('r');
    }
    /*
...
static int bt_emitter_iis_dec_size(void *hdl)
{
    struct bt_emitter_iis_dec_hdl *dec = hdl;
    return cbuf_get_data_size(&dec->cbuf);
}
static int bt_emitter_iis_dec_total(void *hdl)
{
    struct bt_emitter_iis_dec_hdl *dec = hdl;
    return dec->cbuf.total_len;
}

static void bt_emitter_iis_dec_relaese()
{
    if (bt_emitter_iis_dec) {
        audio_decoder_task_del_wait(&decode_task, &bt_emitter_iis_dec->wait);
        clock_remove(AUDIO_CODING_PCM);
        local_irq_disable();
        free(bt_emitter_iis_dec);
        bt_emitter_iis_dec = NULL;
        local_irq_enable();
    }
}

static void __bt_emitter_iis_dec_close(void)
{
    if (bt_emitter_iis_dec && bt_emitter_iis_dec->start) {
        bt_emitter_iis_dec->start = 0;

        pcm_decoder_close(&bt_emitter_iis_dec->pcm_dec);
        bt_emitter_iis_dec->iis_start = 0;
        /* audio_adc_iis_close(&bt_emitter_iis_dec->iis_ch); */
        ...
        audio_adc_del_output_handler(&adc_hdl, &bt_emitter_iis_dec->adc_output);

        audio_mixer_ch_close(&bt_emitter_iis_dec->mix_ch);

        if (bt_emitter_iis_dec->stream) {
            audio_stream_close(bt_emitter_iis_dec->stream);
            bt_emitter_iis_dec->stream = NULL;
        }
    }
}

void bt_emitter_iis_close(void)
{
    if (!bt_emitter_iis_dec) {
        return;
    }
    __bt_emitter_iis_dec_close();
    bt_emitter_iis_dec_relaese();
    clock_set_cur();
    log_i("bt_emitter_iis dec close \n\n ");
}

static void bt_emitter_iis_dec_event_handler(struct audio_decoder *decoder, int argc, int *argv)
{
    switch (argv[0]) {
    case AUDIO_DEC_EVENT_END:
        if (!bt_emitter_iis_dec) {
            log_i("bt_emitter_iis_dec handle err ");
            break;
        }
        if (bt_emitter_iis_dec->id != argv[1]) {
            log_w("bt_emitter_iis_dec id err : 0x%x, 0x%x \n", bt_emitter_iis_dec->id, argv[1]);
            break;
        }
        bt_emitter_iis_close();
        break;
    }
}

static void bt_emitter_iis_dec_out_stream_resume(void *p)
{
    struct bt_emitter_iis_dec_hdl *dec = p;
    audio_decoder_resume(&dec->pcm_dec.decoder);
}

int bt_emitter_iis_dec_start()
{
    int err;
    struct bt_emitter_iis_dec_hdl *dec = bt_emitter_iis_dec;
    struct audio_mixer *p_mixer = &mixer;
    ...
    audio_iis_input_start(TCFG_IIS_INPUT_PORT, TCFG_IIS_INPUT_DATAPORT_SEL, adc_iis_output_handler);

    dec->iis_start = 1;

    dec->skip_cnt = 30;

    pcm_decoder_set_event_handler(&dec->pcm_dec, bt_emitter_iis_dec_event_handler, dec->id);
    pcm_decoder_set_read_data(&dec->pcm_dec, bt_emitter_iis_dec_read, dec);

    // 设置叠加功能
    audio_mixer_ch_open_head(&dec->mix_ch, p_mixer);
    struct audio_mixer_ch_sync_info info = {0};
    info.priv = dec;
    info.get_total = bt_emitter_iis_dec_total;
    info.get_size = bt_emitter_iis_dec_size;
    audio_mixer_ch_set_sync(&dec->mix_ch, &info, 1, 1);
    ...
    entries[entry_cnt++] = &dec->pcm_dec.decoder.entry;
    entries[entry_cnt++] = &dec->mix_ch.entry;
    dec->stream = audio_stream_open(dec, bt_emitter_iis_dec_out_stream_resume);
    audio_stream_add_list(dec->stream, entries, entry_cnt);
    audio_output_set_start_volume(APP_AUDIO_STATE_MUSIC);
    dec->start = 1;
    err = audio_decoder_start(&dec->pcm_dec.decoder);
...
static int bt_emitter_iis_wait_res_handler(struct audio_res_wait *wait, int event)
{
    int err = 0;
    log_i("bt_emitter_iis_wait_res_handler, event:%d\n", event);
    if (event == AUDIO_RES_GET) {
        // 启动解码
        err = bt_emitter_iis_dec_start();
    } else if (event == AUDIO_RES_PUT) {
        // 被打断
        __bt_emitter_iis_dec_close();
    }
...
int bt_emitter_iis_open(void)
{
    ...
    bt_emitter_iis_close();
    int err;
    struct bt_emitter_iis_dec_hdl *dec;
    dec = zalloc(sizeof(*dec));
    if (!dec) {
        return -ENOMEM;
    }
    bt_emitter_iis_dec = dec;
    ...
    dec->wait.priority = 2;
    dec->wait.preemption = 1;
    dec->wait.handler = bt_emitter_iis_wait_res_handler;
    clock_add(AUDIO_CODING_PCM);
    err = audio_decoder_task_add_wait(&decode_task, &dec->wait);
    return err;
}
