...
static void connect_and_connectable_switch(void *_sw);
static void long_time_connect_and_connectable_switch(void *_sw);
static void long_time_connect_and_connectable_switch_with_pair(void *_sw);
...
void bt_long_time_tws_connect_and_connectable_switch(u8 type)
{
    bt_tws_delete_pair_timer();

    if (type) {
        gtws.tws_switch_state = 0;
        long_time_connect_and_connectable_switch_with_pair(0);
    } else {
        long_time_connect_and_connectable_switch(0);
    }
}
...
static void long_time_connect_and_connectable_switch(void *_sw)
{
    ...
    log_info("long_time_switch: %d, state = %x, %d\n", sw, gtws.state,
             bt_user_priv_var.auto_connection_counter);
    ...
    gtws.pair_timer = sys_timeout_add((void *)sw, long_time_connect_and_connectable_switch, timeout);
...
static void long_time_connect_and_connectable_switch_with_pair(void *_sw)
{
    ...
    log_info("pair_long_time_switch: %d, tws_switch_state = %x, %d\n", sw, gtws.tws_switch_state,
             bt_user_priv_var.auto_connection_counter);
    ...
    if (sw) {
        if (gtws.tws_switch_state) {
            tws_creat_connection();
        } else {
            bt_tws_delete_pair_timer();
            tws_open_tws_wait_pair(bt_get_tws_device_indicate(NULL), bt_get_local_name());
        }

        if (++gtws.tws_switch_state > 1) {
            gtws.tws_switch_state = 0;
        }
    ...
    gtws.pair_timer = sys_timeout_add((void *)sw, long_time_connect_and_connectable_switch_with_pair, timeout);
...
void bt_long_time_tws_phone_disconnected(u8 type)
{
    gtws.state &= ~BT_TWS_PHONE_CONNECTED;
    ...
    bt_user_priv_var.auto_connection_counter = 0;
    if (!(gtws.state & BT_TWS_SIBLING_CONNECTED)) {
        bt_long_time_tws_connect_and_connectable_switch(type);
    }
}
...
{
#ifdef TWS_PHONE_LONG_TIME_DISCONNECTED
    if (gtws.state & BT_TWS_UNPAIRED) {
        bt_long_time_tws_phone_disconnected(0);
    } else {
        bt_long_time_tws_phone_disconnected(1);
    }
#else
