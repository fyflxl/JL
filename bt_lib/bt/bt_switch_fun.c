...
/*----------------------------------------------------------------------------*/
/**@brief  蓝牙处于后台模式的时候关闭蓝牙。下次需要重新进入蓝牙初始化
   @param
   @return
   @note 初版应用FM和蓝牙共用RF的情况下，进入FM需要关闭蓝牙
*/
/*----------------------------------------------------------------------------*/
static u8 bt_nobackground_exit();
int bt_background_close_bt_hardward(u8 wait_flag)
{
#if TCFG_BLUETOOTH_BACK_MODE
    printf("exit flag %d, init flag %d\n", __this->exit_flag, __this->init_ok);
    if (!__this->init_ok) {
        __this->close_bt_hw_in_background = 0;
        return 0;
    }
    if (!__this->exit_flag) {
        __this->close_bt_hw_in_background = 0;
    ...
    if (__this->close_bt_hw_in_background == 0) {
        __this->close_bt_hw_in_background = 1;
        bt_nobackground_exit();
    }
    if (__this->exiting && wait_flag) {
        putchar('k');
        os_time_dly(10);
        return -1;
    }
...
#if TCFG_BROADCAST_ENABLE
    broadcast_a2dp_dec_hdl_register();
#endif
}
...
#else
    /*注释掉app_var.goto_poweroff_flag = 0;
     *为了防止关机过程中运行至此处时关机标志
     *被清0导致不能正常关机的情况*/
    /* app_var.goto_poweroff_flag = 0; */
#endif
