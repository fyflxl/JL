...
static int a2dp_drop_frame_timer;

static int broadcast_a2dp_start(void);
static void broadcast_a2dp_stop(void);
static u8 get_a2dp_dec_status(void);
static struct broadcast_params_interface broadcast_ops_cb = {
    .start_original = broadcast_a2dp_start,
    .stop_original = broadcast_a2dp_stop,
    .start = broadcast_a2dp_start,
    .stop = broadcast_a2dp_stop,
    .get_status = get_a2dp_dec_status,
    .sync_params = {
        .source = BROADCAST_SOURCE_BT_BLOCK,
        .input_sample_rate = JLA_CODING_SAMPLERATE,
        .output_sample_rate = JLA_CODING_SAMPLERATE,
        .nch = JLA_CODING_CHANNEL,
        .delay_time = 200 + BROADCAST_TX_DELAY_TIME,
    },
    .dec_params = {
        .nch = JLA_CODING_CHANNEL,
...
static int bt_switch_back(void *p)
{
    ...
    ret = app_task_switch_back();
    if (ret <= 0) {
        __this->call_flag = 0;
        r_printf("bt_switch_back fail");
    }
    return 0;
}
...
void set_a2dp_start_flag(u8 flag)
{
    __this->a2dp_start_flag = flag;
}
...
void bt_set_led_status(u8 status)
{
	...
	log_info("BLE_>%x",TCFG_BLE_DEMO_SELECT);
    if (status) {
        bt_status = status;
    }
...
{
#if (RCSP_MODE)
    extern bool rcsp_set_device_volume(int volume);
    if (rcsp_set_device_volume(volume)) {
        return;
    }
#endif
...
u8 bt_status_event_filter(struct bt_event *bt)
{
    ...
    if (UI_GET_WINDOW_ID() == ID_WINDOW_SYS && bt->event == BT_STATUS_SCO_STATUS_CHANGE) {//通话中界面切换到蓝牙界面下
        UI_MSG_POST("sys_status:event=%4", bt->event);
        return true;
    }
    if (UI_GET_WINDOW_ID() == ID_WINDOW_BT && bt->event == BT_STATUS_SCO_STATUS_CHANGE) {//通话中界面切换到蓝牙界面下
        UI_MSG_POST("bt_status:call_change=%4", 1);
        return true;
    }
#endif
...
void bt_status_connect(struct bt_event *bt)
{
    ...
    rcsp_update_bt_emitter_connect_state(1, __this->last_connecting_addr);
#endif
...
void bt_status_disconnect(struct bt_event *bt)
{
...
#if (RCSP_MODE)
    rcsp_update_bt_emitter_connect_state(0, __this->last_connecting_addr);
#endif
    ...
    if (get_bt_tws_connect_status()) {
        ...
        if (tws_api_get_role() == TWS_ROLE_MASTER) {
#ifdef CONFIG_NEW_BREDR_ENABLE
            bt_tws_led_state_sync(SYNC_LED_STA_PHONE_DISCONN, 400);
#else
...
void bt_status_phone_active(struct bt_event *bt)
{
    ...
    if (bt_user_priv_var.phone_call_dec_begin) {
        ...
        // 注释原因:__this->esco_dump_packet置零会导致iphone7来电(拨号)接听瞬间播放手机自带铃声
...
void bt_status_a2dp_media_start(struct bt_event *bt)
{
    ...
    if (a2dp_drop_frame_timer) {
        sys_timeout_del(a2dp_drop_frame_timer);
        a2dp_drop_frame_timer = 0;
    }
    if (get_broadcast_role() == BROADCAST_ROLE_TRANSMITTER) {
        if (!broadcast_ops_cb.sync_hdl) {
            __this->a2dp_start_flag = 0;
            return ;
        }
        struct a2dp_dec_params a2dp_params = {
            .media_type = bt->value,
            .ch_num = JLA_CODING_CHANNEL,
            .output_priv = broadcast_ops_cb.sync_hdl->bcsync,
            .output_handler = (int (*)(void *, s16 *, int))broadcast_audio_sync_block_input,
            .bcsync = broadcast_ops_cb.sync_hdl->bcsync,
            .format_sync = broadcast_audio_sync_update_source_fmt,
            .resume_sync = broadcast_audio_sync_set_resume_callback,
        };
        a2dp_dec_to_pcm_open(&a2dp_params);
        return ;
    }
#endif
...
#else
    if (bt_user_priv_var.emitter_or_receiver != BT_EMITTER_EN) {
#if (TCFG_BROADCAST_ENABLE && BROADCAST_RECEIVER_CLOSE_EDR_CONN)
...
#if TCFG_BROADCAST_ENABLE
void broadcast_a2dp_dec_hdl_register(void)
{
    broadcast_params_interface_register(&broadcast_ops_cb);
}

static void broadcast_a2dp_drop_frame(void *p)
{
    if (bt_media_is_running()) {
        a2dp_drop_frame_timer = sys_timeout_add(NULL, broadcast_a2dp_drop_frame, 100);
        putchar('D');
        __a2dp_drop_frame(NULL);
    } else {
        local_irq_disable();
...
static int broadcast_a2dp_start(void)
{
    ...
    if ((a2dp_get_status() == BT_MUSIC_STATUS_STARTING) ||
        ...
        struct bt_event event = {
            .value = __this->a2dp_decoder_type,
        };
        bt_status_a2dp_media_start(&event);
    }
}
...
{
    if (bt_media_is_running()) {
        bt_status_a2dp_media_stop(NULL);
        /* a2dp_drop_frame_timer = sys_timeout_add(NULL, broadcast_a2dp_drop_frame, 100); */
    }
}
