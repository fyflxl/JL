...
static int bt_connction_status_event_handler(struct bt_event *bt)
{
    ...
    case BT_STATUS_PHONE_INCOME:
        ...
        result = app_broadcast_deal(BROADCAST_PHONE_START);
        if (result > 0) {
            break;
        }
#endif
...
#if TCFG_BROADCAST_ENABLE
        result = app_broadcast_deal(BROADCAST_PHONE_STOP);
        if (result > 0) {
            break;
        }
#endif
#if TCFG_UNISOUND_ENABLE
        extern void uisound_init(void);
        unisound_init();
#endif
        break;
    ...
    case BT_STATUS_A2DP_MEDIA_START:
        log_info(" BT_STATUS_A2DP_MEDIA_START: %d\n", bt->value);
        __this->a2dp_decoder_type = bt->value;
        if (__this->a2dp_start_flag) {
            break;
        }
        __this->a2dp_start_flag = 1;
#if TCFG_BROADCAST_ENABLE
        result = app_broadcast_deal(BROADCAST_A2DP_START);
        if (result > 0) {
            break;
        }
#endif
...
#if TCFG_BROADCAST_ENABLE
        result = app_broadcast_deal(BROADCAST_A2DP_STOP);
        if (result > 0) {
            break;
        }
#endif
...
#if TCFG_UNISOUND_ENABLE
        extern void unisound_uninit(void);
        unisound_uninit();
#endif
...
#if TCFG_BROADCAST_ENABLE
        app_broadcast_deal(BROADCAST_EDR_DISCONN);
#endif
        bt_hci_event_disconnect(bt) ;
        clock_remove_set(BT_CONN_CLK);
        break;
...
//#endif
#if TCFG_UNISOUND_ENABLE
extern void rgb_led_status_set(u8 red, u8 green, u8 blue, u8 modify);
static void unisound_ui_show(void)
{
    rgb_led_status_set(0, 1, 0, 1);
...
int bt_key_event_handler(struct sys_event *event)
{
    ...
    if (bt_key_event_filter_before() == false) {
		log_debug("bt key_event???:%d %d %d\n", key->value, key->event, key->init);
        return false;
    }
    ...
    case KEY_MEDIA_CODEC_TYPE_SWITCH:
        __this->media_codec_type = !__this->media_codec_type;
        printf("KEY_TYPE_TAKE_PICTURE, %d\n", __this->media_codec_type);
        user_change_media_codec_type_mode(__this->media_codec_type);///
        break;
#endif
    ...
    case KEY_CHANGE_MODE:
        ...
        if (tws_api_get_tws_state() & TWS_STA_SIBLING_CONNECTED) {
            if (tws_api_get_role() == TWS_ROLE_MASTER) {
                ...
                tws_api_sync_call_by_uuid('T', SYNC_CMD_MODE_CHANGE, 500);
#else
...
#if (RCSP_MODE)
    extern int rcsp_bt_key_event_deal(int key_event, int ret);
    ret = rcsp_bt_key_event_deal(key_event, ret);
#endif
    return ret;
}
