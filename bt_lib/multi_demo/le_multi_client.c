...
static void client_report_ext_adv_data(u8 *report_pt, u16 len)
{
    ...
    find_remoter = resolve_adv_report(evt->Address, evt->Data_Length, evt->Data, evt->RSSI);

    if (find_remoter) {
        if (force_seach_onoff && force_seach_rssi > evt->RSSI) {
            log_info("match but rssi fail!!!:%d,%d\n", force_seach_rssi, evt->RSSI);
            return;
        }

        log_info("\n*********ext create_connection***********\n");
        log_info("***remote type %d, addr:", evt->Address_Type);
        log_info_hexdump(evt->Address, 6);
        bt_ble_scan_enable(0, 0);
        client_create_connect_api(evt->Address, evt->Address_Type, 0);
        log_info("*create_finish\n");
    }
}
...
const le_ext_scan_param_lite_t ext_scan_param = {
    .Own_Address_Type = 0,
