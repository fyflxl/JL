...
#endif
        rcsp_function_update(COMMON_FUNCTION, BIT(COMMON_FUNCTION_ATTR_TYPE_PHONE_SCO_STATE_INFO));
        break;
...
static void rcsp_bt_tws_event_handler(struct bt_event *bt)
{
    int role = bt->args[0];
...
int sys_event_handler_specific(struct sys_event *event)
{
    ...
    case SYS_BT_EVENT:
        if ((u32)event->arg == SYS_BT_EVENT_TYPE_CON_STATUS) {
            rcsp_bt_status_event_handler(&event->u.bt);
        } else if ((u32)event->arg == SYS_BT_EVENT_TYPE_HCI_STATUS) {
            rcsp_hci_event_handler(&event->u.bt);
        }
#if TCFG_USER_TWS_ENABLE
        else if (((u32)event->arg == SYS_BT_EVENT_FROM_TWS)) {
            rcsp_bt_tws_event_handler(&event->u.bt);
        } else if (((u32)event->arg == SYS_BT_EVENT_FROM_APP_OPT_TWS)) {
            rcsp_app_opt_tws_event_handler(&event->u.bt);
        }
#endif
#if OTA_TWS_SAME_TIME_ENABLE
...
int rcsp_user_spp_state_specific(u8 packet_type)
{
    ...
    case 1:
        ...
        extern void rcsp_spp_setting(void);
        rcsp_spp_setting();
        break;
    case 2:
