...
static u32 target_feature_attr_sys_info(void *priv, u8 attr, u8 *buf, u16 buf_size, u32 offset)
{
    ...
    extern void rcsp_get_cur_dev_vol_info(u8 * vol_info);
    extern void rcsp_get_max_vol_info(u8 * vol_info);
    ...
    if (get_remote_vol_sync() || (1 == rcspModel->A_platform)) {
...
static u32 target_feature_ble_only(void *priv, u8 attr, u8 *buf, u16 buf_size, u32 offset)
{
    ...
    memcpy(taddr_buf + 1, (void *)bt_get_mac_addr(), 6);
#else
