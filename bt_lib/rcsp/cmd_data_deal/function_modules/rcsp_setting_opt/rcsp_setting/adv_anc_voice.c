...
extern u16 get_adv_setting_event_flag(void);
...
#if TCFG_USER_TWS_ENABLE
        if ((-1 == value_old) || (get_bt_tws_connect_status() && (count_the_num_of_positions(get_adv_setting_event_flag()) > 1))) {
#else
        if (-1 == value_old) {
#endif
...
void deal_anc_voice(u8 *anc_setting, u8 write_vm, u8 tws_sync)
{
    ...
    if (get_bt_tws_connect_status()) {
        if (TWS_ROLE_MASTER == tws_api_get_role()) {
            anc_voice_state_update(anc_setting);
        } else if (0 == tws_sync) {
            anc_voice_state_update(anc_setting);
        }
    } else
