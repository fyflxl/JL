...
*/
/*----------------------------------------------------------------------------*/
extern int rcsp_file_transfer_watch_opt(u8 flag, char *root_path);
static void file_transfer_watch_opt(u32 dev_handle, u8 flag)
{
    if (BS_FLASH == dev_handle
        || BS_FLASH_2 == dev_handle) {
        char *root_path = NULL;
        if (__this->dev) {
...
void file_delete_start(u8 OpCode_SN, u8 *data, u16 len)
{
    ...
    if (__this->fsn == NULL) {
        ...
        if (__this->dev) {
            __this->fsn = dev_manager_scan_disk(__this->dev, NULL, scan_parm, 0, (struct __scan_callback *)&scan_cb);
            if (__this->fsn == NULL) {
                free(__this);
                __this = NULL;
                goto __err;
