...
#include "debug.h"
#endif
#define log_info(x, ...)       printf("[APP_CHARGE]" x "\r\n", ## __VA_ARGS__)
...
static u8 charge_in_deal_flag = 0;
...
void charge_ldo5v_off_deal(void)
{
    ...
    if (!charge_in_deal_flag) {
        r_printf("charge_in_deal_flag is 0");
        return;
    }
...
int app_charge_event_handler(struct device_event *dev)
{
    ...
    case CHARGE_EVENT_LDO5V_IN:
        ...
        while (detect_timeout > 0) {
            u8 app = app_get_curr_task();
            if (app == APP_IDLE_TASK) {
                g_printf("cur mode is idle, direct charge");
                break;
            }
            otg_status = usb_otg_online(0);
            if ((otg_status == SLAVE_MODE) || (otg_status == CHARGE_MODE)) {
                break;
            }
            os_time_dly(2);
            detect_timeout -= 20;
