...
#endif/*TCFG_AUDIO_OUT_EQ_ENABLE*/
#if (RCSP_ADV_EN)&&(TCFG_DRC_ENABLE == 0)
...
const int speex_max_framelen = 399; //必须大于等于一帧的长度。quality等于4，帧长20；等于5，帧长28。最长399

#if TCFG_DEC2TWS_ENABLE || SOUNDCARD_ENABLE || TCFG_MIC_EFFECT_ENABLE || RECORDER_MIX_EN || TCFG_USER_EMITTER_ENABLE ||TCFG_DYNAMIC_EQ_ENABLE
...
const int JLA_SUPPORT_CH = JLA_CODING_CHANNEL;     //jla解码输入通道数 1：单声道输入， 2:双声道输入(br30可支持2)
const int JLA_DMS_VAL = JLA_CODING_FRAME_LEN;      //单位ms, 【只支持 25,50,100】
...
const int CONFIG_BT_AUDIO_SYNC_MODE = 1;
#else
const int CONFIG_BT_AUDIO_SYNC_MODE = 0;
#endif
const int USE_PREFILT_EN = 1;// 影响 EFFECT_3D_LRDRIFT2的效果，跟平台无关。该变量后续会删
/**
...
const char log_tag_const_v_AUDIO_STREAM AT(.LOG_TAG_CONST) = CONFIG_DEBUG_LIB(1);
const char log_tag_const_c_AUDIO_STREAM AT(.LOG_TAG_CONST) = CONFIG_DEBUG_LIB(1);
const char log_tag_const_i_AUDIO_STREAM AT(.LOG_TAG_CONST) = CONFIG_DEBUG_LIB(1);
