...
static int sys_status_handler(const char *type, u32 arg)
{
    UI_HIDE_CURR_WINDOW();
...
static const struct uimsg_handl ui_msg_handler[] = {
    { "sys_status",        sys_status_handler     }, /* 设置音量 */
    { NULL, NULL},      /* 必须以此结尾！ */
};
...
static int sys_mode_onchange(void *ctr, enum element_change_event e, void *arg)
{
    ...
    case ON_CHANGE_INIT:
        puts("\n***sys_mode_onchange***\n");
        /* key_ui_takeover(1); */
        ui_register_msg_handler(ID_WINDOW_SYS, ui_msg_handler);
        /*
          * 注册APP消息响应
          */
        break;
...
static void system_update_text(void *p)
{
    if (update_status != UPDATA_NON) {
        ui_text_show_index_by_id(NO_UPDATEFAIL_TXT, 1);
    } else {
        ui_text_show_index_by_id(NO_UPDATEFAIL_TXT, 2);
    }
}

static int system_update_onchange(void *ctr, enum element_change_event e, void *arg)
{
    struct window *window = (struct window *)ctr;
    static int bt_status_timer = 0;
    switch (e) {
    case ON_CHANGE_INIT:
        puts("\n***system_update_onchange***\n");
        ui_set_call(system_update_text, 0);
        break;
    case ON_CHANGE_RELEASE:
        break;
    default:
...
REGISTER_UI_EVENT_HANDLER(SYSTEM_UPDATE)
.onchange = system_update_onchange,
 .onkey = NULL,
  .ontouch = NULL,
};
