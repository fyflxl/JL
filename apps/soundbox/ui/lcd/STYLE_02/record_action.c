...
static int record_layout_onchange(void *ctr, enum element_change_event e, void *arg)
{
    ...
    switch (e) {
    case ON_CHANGE_INIT:
        UI_MSG_POST("record_start:first_show=%4", 1); //录音模式上电显示空白路径
        if (!__this->timer) {
            __this->timer = sys_timer_add(NULL, record_check_add, 500);
            __this->rec_min_time = REC_MIN_TIME;//上电后立即录音不会丢失按键消息
        }
        break;
...
static int record_layout_onkey(void *ctr, struct element_key_event *e)
{
    ...
    case KEY_MENU:
        ...
        break;
    case KEY_ENC:
        if (__this->rec_min_time < REC_MIN_TIME) {//避免双击响应两次
            printf("\n--func=%s, line=%d\n", __FUNCTION__, __LINE__);
            break;
        }
        app_task_put_key_msg(KEY_ENC_START, 0);
        __this->rec_min_time = 0;
        break;
    case KEY_UP:
        break;
    case KEY_DOWN:
...
static void record_check_add(void *ptr)
{
    ...
    if (__this->rec_min_time < REC_MIN_TIME) {
        __this->rec_min_time++;
    }
    ...
    if (1) {
        if (!__this->start) { //刚开始进行录音或者总系统设置出来后显示录音盘符,名字,正在录音文本以及播放标志
            ...
            UI_MSG_POST("record_start:show_filename=%4:dev=%4", 1, logo);
            __this->start  = 1;
            __this->second = 0;
        }
        ...
        if (__this->start == 1) { //显示录音时间
            ...
            __this->second = 0;//recorder_get_encoding_time();
            printf("\n--func=%s, line=%d,%d\n", __FUNCTION__, __LINE__, __this->second);
            if (__this->second < 0) {
                __this->second = 0;
            }
        }
