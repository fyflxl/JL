...
static void ui_sys_enter_soft_poweroff(void *priv)
{
#if TCFG_APP_BT_EN
    if ((get_call_status() == BT_CALL_OUTGOING)
        ...
        if (system_auto_close_timer) {
            system_auto_close_timer = sys_s_hi_timerout_add(NULL, ui_sys_enter_soft_poweroff, (table_system_auto_close[__this->cur_auto_close_time ] * 1000));
        }
    } else {
        sys_enter_soft_poweroff(priv);
    }
#else
    sys_enter_soft_poweroff(priv);
#endif
...
void set_auto_poweroff_timer(int sel_item)
{
    ...
    if (system_auto_close_timer) {
        ...
        system_auto_close_timer = sys_s_hi_timerout_add(NULL, ui_sys_enter_soft_poweroff, (time * 1000));
    }
    sys_param_write2vm();
}
