...
int aec_cfg_online_init()
{
    ...
    if ((aec_update->cfg.aec_mode & AEC_MODE_ADVANCE) == AEC_MODE_ADVANCE) {
        aec_update->cfg.aec_mode = 2;
    } else if ((aec_update->cfg.aec_mode & AEC_MODE_ADVANCE) == AEC_MODE_REDUCE) {
        aec_update->cfg.aec_mode = 1;
    } else {
        aec_update->cfg.aec_mode = 0;
    }
...
int aec_cfg_online_update_fill(void *cfg, u16 len)
{
    if (aec_update && aec_update->update) {
        ...
        AEC_CONFIG *aec_cfg = (AEC_CONFIG *)cfg;
        if (aec_cfg->aec_mode == 2) {
            aec_cfg->aec_mode &= ~AEC_MODE_ADVANCE;
            aec_cfg->aec_mode |= AEC_MODE_ADVANCE;
        } else if (aec_cfg->aec_mode == 1) {
            aec_cfg->aec_mode &= ~AEC_MODE_ADVANCE;
            aec_cfg->aec_mode |= AEC_MODE_REDUCE;
        } else {
            aec_cfg->aec_mode &= ~AEC_MODE_ADVANCE;
        }
        if (aec_update->agc_en) {
            aec_cfg->aec_mode |= AGC_EN;
        }
#endif/*TCFG_AUDIO_DUAL_MIC_ENABLE*/
        return len;
    }
