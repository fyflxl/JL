...
int audio_aec_open(u16 sample_rate, s16 enablebit, int (*out_hdl)(s16 *data, u16 len))
{
    ...
    /*参考数据通道数,内部读dac数据用到*/
    aec_param->ref_channel = 1;
    /*adc回采作为参考数据*/
#if (TCFG_CVP_REF_IN_ADC_ENABLE)
    aec_param->adc_ref_en = 1;
#else
    aec_param->adc_ref_en = 0;
...
#if  (TCFG_AUDIO_SMS_ENABLE == SMS_TDE)//SMS_DISABLE
    aec_param->EnableBit |= TDE_EN;
...
{
    if (aec_hdl && aec_hdl->start) {
#if (AEC_TOGGLE && TCFG_CVP_REF_IN_ADC_ENABLE)
        aec_in_ref_data((u8 *)buf, len);
#endif/*AEC_TOGGLE*/
    }
}
