...
/* #define TCFG_LFN_PATH_EN 1 */
#if TCFG_LFN_EN
static u8 music_file_name[128] = {0}; //长文件名
u16    music_file_name_len = 0;
#if TCFG_LFN_PATH_EN
static u8 music_path[128] = {0}; //路径名
u16    music_path_len = 0;
#endif
#else
...
static void music_player_play_success(void *priv, int parm)
{
    ...
    int unicode = 0;
#if TCFG_LFN_PATH_EN
    music_path_len = fget_path(music_player_get_file_hdl(), music_player_get_vfscan_hdl(), music_path, sizeof(music_path), 1);
    if ((music_path[0] == '\\') && (music_path[1] == 'U')) {
        unicode = 1;
        log_i("cur path  = %s, len = %d, unicode = %d\n", music_path + 2, music_path_len - 2, unicode);
        /* put_buf(music_path, music_path_len); */
    } else {
        log_i("cur path  = %s, len = %d, unicode = %d\n", music_path, music_path_len, unicode);
    }
#endif
    ...
    if ((music_file_name[0] == '\\') && (music_file_name[1] == 'U')) {
        ...
        char *utf8 = zalloc(2 * sizeof(music_file_name));
        int file_len = Unicode2UTF8(utf8, (u16 *)music_file_name + 1, music_file_name_len / 2);
        y_printf(">>>[test]:filename = %s, len - %d\n", utf8, file_len);
        if (utf8) {
            free(utf8);
            utf8 = NULL;
        }
...
static u8 music_key_event_filter(int key_event)
{
#if (TCFG_BROADCAST_ENABLE)
...
static int music_key_event_opr(struct sys_event *event)
{
    ...
    if (music_key_event_filter(msg[0]) == true) {
        return true;
    }
    ...
    case  KEY_MUSIC_PP:
        ...
        } else if (music_player_get_play_status() == FILE_DEC_STATUS_STOP) {
...
#if TCFG_BROADCAST_ENABLE
    broadcast_file_dec_hdl_register();
#endif
