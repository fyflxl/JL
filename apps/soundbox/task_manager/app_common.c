...
int app_common_key_msg_deal(struct sys_event *event)
{
    ...
    case KEY_CHANGE_MODE:
        ...
        extern void unisound_init();
        extern void unisound_uninit();
        extern u8 get_unisound_init_ok();
        rgb_led_status_set(0, 1, 0, 1);
        tone_play_with_callback_by_name(tone_table[IDEX_TONE_UNISOUND], 1, NULL, NULL);
        if (get_unisound_init_ok()) {
            unisound_uninit();
        } else {
            unisound_init();
        }
    ...
    break;
#if defined(TCFG_BROADCAST_ENABLE) && (TCFG_BROADCAST_ENABLE)
