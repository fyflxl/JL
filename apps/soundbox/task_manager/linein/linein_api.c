...
#if TCFG_BROADCAST_ENABLE
static u8 get_linein_onoff(void);
static int broadcast_linein_start(void);
static void broadcast_linein_stop(void);
static void broadcast_linein_pp(void);

static struct broadcast_params_interface broadcast_ops_cb = {
    .start_original = linein_start,
    .stop_original = linein_stop,
    .start = broadcast_linein_start,
    .stop = broadcast_linein_stop,
    .get_status = get_linein_onoff,
    .sync_params = {
        ...
        .delay_time = BROADCAST_TX_DELAY_TIME,//45,
    },
    .dec_params = {
        .nch = JLA_CODING_CHANNEL,
...
#if TCFG_BROADCAST_ENABLE
void broadcast_linein_hdl_register(void)
{
...
static int broadcast_linein_start(void)
{
    ...
    struct broadcast_linein_params linein = {
        .sample_rate = JLA_CODING_SAMPLERATE,
        .gain = 3,//10,
...
{
    if (__this->onoff == 0) {
        /* broadcast_linein_start(); */
        __this->onoff = 1;
        app_broadcast_deal(BROADCAST_MUSIC_START);
    } else {
        /* broadcast_linein_stop(); */
        __this->onoff = 0;
