...
#define USE_CONFIG_COMBINE_VOL_SETTING       0		  //联合音量读配置
...
//-----------------------------CFG_BLE_NAME--------------------------------------//
    ...
    AEC_CONFIG aec;
#if(TCFG_AUDIO_CVP_NS_MODE == CVP_ANS_MODE)
    log_info("sms_ans config:");/*单mic传统降噪*/
    ret = syscfg_read(CFG_AEC_ID, &aec, sizeof(aec));
#else
    log_info("sms_dns config:");/*单mic神经网络降噪*/
    ...
    if (ret > 0) {
        ...
        log_info("max vol:%d default vol:%d tone vol:%d\n", audio_cfg.max_sys_vol, audio_cfg.default_vol, audio_cfg.tone_vol);
#else
    {
#endif
