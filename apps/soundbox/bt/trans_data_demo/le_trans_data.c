...
#include "rcsp_bluetooth.h" // 该头文件属于旧RCSP模块，后续开发需要废弃接入新的RCSP模块，by lingxuanfeng
...
static void ancs_remove_message(u8 *packet, u16 size)
{
...
void ble_profile_init(void)
{
    ...
    if ((!config_le_sm_support_enable) || (!config_le_gatt_client_num)) {
        printf("ancs&ams need sm and client support!!!\n");
        ASSERT(0);
    }
    //setup GATT client
    gatt_client_init();
