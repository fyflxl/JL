...
static char *gap_device_name = "JL_ble_111";
...
{
	if(ble_work_state==BLE_ST_CONNECT)
	{
		return 1;
	}
	else
...
//参考识别
static void rcsp_att_check_remote_result(u16 con_handle, remote_type_e remote_type)
{
    log_info("rcsp %02x:remote_type= %02x\n", con_handle, remote_type);
...
static void cbk_packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
	...
	if(packet_type)
		log_info("---->%s,%x,%x,%x\n",__func__,packet_type,channel,hci_event_packet_get_type(packet));
    switch (packet_type) {
    case HCI_EVENT_PACKET:
        switch (hci_event_packet_get_type(packet)) {
...
static uint16_t att_read_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
	...
	#if 1
    case ATT_CHARACTERISTIC_2a54_01_VALUE_HANDLE:
		if (buffer) {
		buffer[0] = 00;
		buffer[1] = RSCS_FLAG;
		att_value_len =2;
...
static int att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    case ATT_CHARACTERISTIC_2a00_01_VALUE_HANDLE:
        ...
        log_info("\n----write gap_name:%x,%x-%x,%x\n",gap_device_name[0],gap_device_name[1],gap_device_name[offset],gap_device_name[offset+1]);
		buffer[0] = 75;
		buffer[1] = 80;
		buffer_size = 1;
		app_send_user_data(ATT_CHARACTERISTIC_2a37_01_VALUE_HANDLE, buffer, buffer_size, ATT_OP_NOTIFY);
        break;
...
{
    log_info("ble profile init-----1\n");
    le_device_db_init();

#if TCFG_BLE_BRIDGE_EDR_ENALBE
    ble_sm_setup_init(IO_CAPABILITY_DISPLAY_ONLY, SM_AUTHREQ_SECURE_CONNECTION | SM_AUTHREQ_BONDING, 7, RCSP_TCFG_BLE_SECURITY_EN);
#else
    ble_sm_setup_init(IO_CAPABILITY_NO_INPUT_NO_OUTPUT, SM_AUTHREQ_MITM_PROTECTION | SM_AUTHREQ_BONDING, 7, RCSP_TCFG_BLE_SECURITY_EN);
#endif
...
void ble_profile_again_init(u8 enable)
{
    u8 tmp_ble_addr[6];
    if (enable) {
        log_info("ble profile again init\n");
        /* setup ATT server */
        att_server_init(profile_data, att_read_callback, att_write_callback);
        att_server_register_packet_handler(cbk_packet_handler);
        /* gatt_client_register_packet_handler(packet_cbk); */
...
contrl_adv:
    ...
    cur_state =  rcsp_get_ble_work_state();
    log_info("rcsp_get_ble_work_state  %d\n", cur_state);
    switch (cur_state) {
    case BLE_ST_ADV:
    case BLE_ST_IDLE:
    case BLE_ST_INIT_OK:
...
static int app_send_user_data_do(void *priv, u8 *data, u16 len)
{
    ...
    return app_send_user_data(ATT_CHARACTERISTIC_2a37_01_VALUE_HANDLE, data, len, ATT_OP_NOTIFY);
}
...
uint8_t ble_buffer[10];

void ble_send_heart_data(type_OHRL_output* DATAbuf)
{
	ble_buffer[0]=06;
	ble_buffer[1]=DATAbuf->heartrate;
	app_send_user_data(ATT_CHARACTERISTIC_2a37_01_VALUE_HANDLE, ble_buffer, 2, ATT_OP_NOTIFY);
	log_info("heart=%dn",ble_buffer[1]);
}
void ble_send_pedo_data(type_PedoSummary* buff)
{
	ble_buffer[0] = RSCS_FLAG;
	ble_buffer[1] = buff->pace;//pace low MIN/KM,COUNT
	ble_buffer[2] = 0;//pace high
	ble_buffer[3] = buff->instantsteps;//cadence  ok
	...
	app_send_user_data(ATT_CHARACTERISTIC_2a53_01_VALUE_HANDLE, &ble_buffer[0], 8, ATT_OP_NOTIFY);//17
	log_info("pace=%d,can=%d\n",ble_buffer[1],ble_buffer[3]);
...
void ble_rcsp_test_pro(void)
{
	...
	if((ble_buffer[1]>=120)||(ble_buffer[1]<75))
	{
		ble_buffer[1]=90;
	}
	else
		ble_buffer[1]++;
	app_send_user_data(ATT_CHARACTERISTIC_2a37_01_VALUE_HANDLE, ble_buffer, 2, ATT_OP_NOTIFY);
	ble_buffer[2] = RSCS_FLAG;
	ble_buffer[3] = 3;//10;//1 //pace low MIN/KM,COUNT
	ble_buffer[4] = 1;//11;//2 //pace high
	ble_buffer[5] = 120;//cadence  ok
	
	ble_buffer[6] = 0; //speed
	//ble_buffer[7] = 0; //Cadence
	ble_buffer[8] = 0; //Cadence
	ble_buffer[9] = 0; //Cadence
	...
	app_send_user_data(ATT_CHARACTERISTIC_2a53_01_VALUE_HANDLE, &ble_buffer[2], 8, ATT_OP_NOTIFY);//17
	log_info("%d,%d\n",ble_buffer[1],ble_buffer[7]);
}
