...
#if TCFG_BROADCAST_ENABLE
extern struct __breakpoint *breakpoint;

static int broadcast_file_dec_start(void *priv);
static void broadcast_file_dec_stop(void);
static u8 get_file_dec_status(void);
static struct broadcast_params_interface broadcast_ops_cb = {
    .start_original = broadcast_file_dec_start,
    .stop_original = broadcast_file_dec_stop,
    .start = broadcast_file_dec_start,
    .stop = broadcast_file_dec_stop,
    .get_status = get_file_dec_status,
    .sync_params = {
        .source = BROADCAST_SOURCE_FILE_BLOCK,
...
void  *get_sync_hdl(void)
{
    if (broadcast_ops_cb.sync_hdl) {
        return broadcast_ops_cb.sync_hdl->bcsync;
    }
    return NULL;
}
#endif
...
int music_player_decode_start(FILE *file, struct audio_dec_breakpoint *dbp)
{
    ...
    if (get_broadcast_role() == BROADCAST_ROLE_TRANSMITTER) {
        ...
        struct file_dec_params file_params = {
            .bp = dbp,
            .file = file,
        ...
        ret = file_dec_to_pcm_open(&file_params);
    } else
#endif
    {
...
#if TCFG_BROADCAST_ENABLE
void broadcast_file_dec_hdl_register(void)
{
