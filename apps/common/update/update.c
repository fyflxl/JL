...
/* TODO */
int func_get_rf_info(u8 *buf)
{
    struct rf_pa_info_t info = {0};
    info.pwr_level = 1;
    info.tx_io = 5;
    info.rx_io = 0xaa;
...
static void update_param_content_fill(int type, UPDATA_PARM *p, void (*priv_param_fill_hdl)(UPDATA_PARM *P))
{
    ...
    ext_len = func_get_rf_info(ext_data);
    y_printf(">>>[test]:ext_len = 0x%x\n", ext_len);
    put_buf(ext_data, ext_len);
    update_param_ext_fill(p, EXT_RF_PA_INFO, ext_data, ext_len);
    /* u8 *data = update_param_ext_get(p, EXT_RF_PA_INFO); */
    /* y_printf("\n >>>[test]:func = %s,line= %d\n",__FUNCTION__, __LINE__); */
    /* put_buf(data, ext_len); */
    if (ext_data) {
        free(ext_data);
        ext_data = NULL;
    }
...
extern void app_broadcast_close();
