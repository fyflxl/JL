...
#define TCFG_GPS_DEV_ENABLE  0
#if defined(TCFG_GPS_DEV_ENABLE) && TCFG_GPS_DEV_ENABLE

//BAUD(0~8):4800, 9600, 19200, 38400, 57600, 115200, 230400, 460800, 921600
#define GPS_SET_BAUD         6     //设置GPS模块波特率0~8 (目前0,无法初始化)
...
u8 skytra_cfg_message_interval();//配置NMEA消息输出间隔及消息种类
u8 skytra_cfg_message_type(u8 type);//配置输出消息类型

void uart_gps_init();
void get_gps_message_continoue();
void get_gps_message_single();
