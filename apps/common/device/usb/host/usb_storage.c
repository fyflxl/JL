...
void udisk_speed_test_init()
{
    ...
    wr_KBps_avg = 0;
    rd_KBps_avg = 0;
    timer_irq_idx = IRQ_TIME5_IDX;
    for (i = 0; i < 6; i++) {
        pTimer = tregs[5 - i];
        if ((pTimer->CON & 3) == 0) {
            timer_irq_idx = IRQ_TIME5_IDX - i;
            break;
        }
    }
    if (i == 6) {
...
void usb_host_test2()
{
    ...
    err = usb_stor_open("udisk0", &device, NULL);
    if (err) {
        return;
    }
    //测试起始地址
...
static void udisk_test_event_handler(struct sys_event *e)
{
    char *usb_msg;
    if (e->arg == (void *)DEVICE_EVENT_FROM_OTG) {
        usb_msg = (char *)e->u.dev.value;
        if (usb_msg[0] == 'h') {
            if (e->u.dev.event == DEVICE_EVENT_IN) {
                if (usb_host_mount(0, 3, 56, 50)) {
                    return;
                }
                usb_host_test2();
...
SYS_EVENT_HANDLER(SYS_DEVICE_EVENT, udisk_test_event_handler, 0);
#endif
#endif
