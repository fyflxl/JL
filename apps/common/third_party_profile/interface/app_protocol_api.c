...
static const struct callback_register_t gfps_callback = {
    ...
    .tx_resume = NULL,
    .rx_resume = NULL,
    .get_battery = gfps_get_battery_callback_register,
};
static const struct app_ctrl_operation_t gfps_ctrl = {
    ...
    .update_tws_state_to_lib = gfps_update_tws_state_to_lib,
...
//lib收到控制消息后，会传消息上来处理
static int gfps_special_message(int id, int opcode, u8 *data, u32 len)
{
    printf("gfps_special_message:0x%x", opcode);
    int ret = 0;
    switch (opcode) {
...
static int app_protocol_gfps_init(int match_id)
{
    ...
    if (app) {
        ...
        if (app) {
            ...
            app->loop = NULL;
            app->tx_loop = NULL;
            app->special_message = gfps_special_message;
