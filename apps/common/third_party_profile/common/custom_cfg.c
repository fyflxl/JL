...
#elif (!defined(SMART_BOX_EN) && !defined(RCSP_MODE) && RCSP_ADV_EN)
...
typedef struct _update_serial_num_cfg {
    u16 len;
    u8 data[32];
} update_serial_num_cfg_t;
static update_serial_num_cfg_t update_serial_num_cfg = {
    .len = 0,
};
...
#endif
    [6] = {
        (u8 *)"serial_num", (u8 *) &update_serial_num_cfg.data, sizeof(update_serial_num_cfg.data), &update_serial_num_cfg.len
    },
};
...
static void custom_cfg_file_init(void)
{
    static u8 init = 0;
    if (init) {
        return ;
...
#if VER_INFO_EXT_COUNT
                custom_cfg_item_write(CFG_ITEM_VER_INFO, item_data, item_len);
                /* } else if (0 == strcmp((const char *)item_name, "serial_num")) { */
                /*     custom_cfg_item_write(CFG_ITEM_SERIAL_NUM, item_data, item_len); */
#endif
            }
        }
    }
...
extern u8 *ble_get_scan_rsp_ptr1(u8 *len);
extern u8 *ble_get_adv_data_ptr1(u8 *len);
extern int le_controller_get_mac(void *addr);
extern u8 *ble_get_gatt_profile_data1(u16 *len);
extern const char *bt_get_local_name();
...
{
    custom_cfg_file_init();
    u8 *item_name = cfg_item_description[6].item_name;
    u8 *item_data = cfg_item_description[6].item_data;
    u16 item_len =  cfg_item_description[6].item_len;
    u16 *real_len = cfg_item_description[6].real_len;
    *data = item_data;
    *len = *real_len;
    return 0;
}
