...
void uart_event_handler(struct sys_event *e)
{
    ...
    if (user_var.charg_flag==false) { //(!strcmp(e->arg, "uart_rx_outtime")) {
        if (e->u.dev.event == DEVICE_EVENT_CHANGE) {
            //printf("uart event: %d,%x\n", e->u.dev.event,e->u.dev.value);
            _uart_bus = (const uart_bus_t *)e->u.dev.value;
            uart_rxcnt = _uart_bus->read(uart_rxbuf, sizeof(uart_rxbuf), 0);
		#if 1
            if (uart_rxcnt) {
				htk_getDataToCommun(uart_rxbuf,uart_rxcnt);
                //printf("get_buffer:\n");
...
void uart_u_task(void *param)
{
    ...
    while (1) {
        ...
        os_time_dly(100);             //一般发送512byte,DMA模式需要2.7ms
    }
	#endif
	//uartTickSend();
}
...
void uartSendInit()
{
    ...
    u_arg.is_9bit = 0;
     ...
     gpio_set_die(PCM_UART1_RX_PORT, 1);

		//gpio_direction_output(PCM_UART1_TX_PORT, 1);
		//gpio_set_fun_output_port(arg->tx_pin, tx_ch, 1, 1, LOW_POWER_FREE);

		//gpio_direction_input(PCM_UART1_RX_PORT);
		gpio_set_pull_up(PCM_UART1_RX_PORT, 1);
		gpio_set_die(PCM_UART1_RX_PORT, 1);
		//gpio_set_fun_input_port(arg->rx_pin, rx_pfun, LOW_POWER_FREE);

    printf("uart_dev_open() ...\n");
    uart_bus = uart_dev_open(&u_arg);
    printf("comming %s,%d\n", __func__, __LINE__);
    if (uart_bus != NULL) {
        printf("success\n");
       
        //usr_timer_add(NULL, uartTickSend, 100, 0); //100ms
	...
	//usr_timer_add(NULL, uartTickSend, 1000, 1);//usr_timer定时扫描增加接口
}
