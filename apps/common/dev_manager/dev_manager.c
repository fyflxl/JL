...
{
	//dev_manager_find_active(1);
	struct __dev *devcur= dev_manager_find_active(0);
	struct __dev_reg *dev = devcur->parm;
	char *logo = dev_manager_get_phy_logo(devcur);//最后活动设备
	//printf("----> dev_path = %s\n", );
#if 1
    //struct storage_dev *dev = storage_dev_last();
    printf("%s,%d, %s", __func__, __LINE__,dev_manager_get_root_path_by_logo(logo));
    if (!dev) {
        printf("%s,%d ", __func__, __LINE__);
        return ;
    }
    printf("%s,%d ", __func__, __LINE__);
    printf("devname:%s, pa:%s, fs:%s ", dev->name, dev->storage_path, dev->fs_type);
