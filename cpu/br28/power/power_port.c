...
void board_set_soft_poweroff_common(void *priv)
{
	...
	gpio_direction_input(TCFG_IOKEY_PREV_ONE_PORT);
    gpio_set_pull_down(TCFG_IOKEY_PREV_ONE_PORT, 0);
    gpio_set_pull_up(TCFG_IOKEY_PREV_ONE_PORT, 1);
    gpio_set_die(TCFG_IOKEY_PREV_ONE_PORT, 1);

	gpio_direction_input(TCFG_IOKEY_NEXT_ONE_PORT);
    gpio_set_pull_down(TCFG_IOKEY_NEXT_ONE_PORT, 0);
    gpio_set_pull_up(TCFG_IOKEY_NEXT_ONE_PORT, 1);
    gpio_set_die(TCFG_IOKEY_NEXT_ONE_PORT, 1);
}
#if 1

void amp_mute_off(void *priv)
{
	gpio_set_output_value(TCFG_AUDIO_DAC_PA_PORT1, 1);//AMP MUTE OFF
}

void amp_power_on(void *priv)
{
	gpio_set_output_value(TCFG_IO_AMP_POWER, 0);//AMP power on
	usr_timeout_add(NULL, amp_mute_off, 500, 1);//100ms
}


void power_on_port_init(void)
{
	#if (TCFG_SD0_ENABLE || TCFG_SD1_ENABLE)
	gpio_set_pull_up(TCFG_SD0_POWER, 1);
    gpio_set_pull_down(TCFG_SD0_POWER, 0);
    gpio_set_die(TCFG_SD0_POWER, 1);
	gpio_set_direction(TCFG_SD0_POWER, 0);
	gpio_set_output_value(TCFG_SD0_POWER, 1);//SD POWER ON
	#endif
	gpio_set_pull_up(TCFG_IO_AMP_POWER, 1);
    gpio_set_pull_down(TCFG_IO_AMP_POWER, 0);
    gpio_set_die(TCFG_IO_AMP_POWER, 1);
	gpio_set_direction(TCFG_IO_AMP_POWER, 0);
	gpio_set_output_value(TCFG_IO_AMP_POWER, 1);//AMP POWER Off

	gpio_set_pull_up(TCFG_AUDIO_DAC_PA_PORT1, 1);
    gpio_set_pull_down(TCFG_AUDIO_DAC_PA_PORT1, 0);
    gpio_set_die(TCFG_AUDIO_DAC_PA_PORT1, 1);
	gpio_set_direction(TCFG_AUDIO_DAC_PA_PORT1, 0);
	gpio_set_output_value(TCFG_AUDIO_DAC_PA_PORT1, 0);//AMP MUTE On

	usr_timeout_add(NULL, amp_power_on, 500, 1);//100ms

#if 1
	gpio_set_pull_up(TCFG_IO_SHIP_EN, 1);
    gpio_set_pull_down(TCFG_IO_SHIP_EN, 0);
    gpio_set_die(TCFG_IO_SHIP_EN, 1);
	gpio_set_direction(TCFG_IO_SHIP_EN, 0);
	gpio_set_output_value(TCFG_IO_SHIP_EN, 1);//TCFG_IO_SHIP_EN ship enable
#endif
