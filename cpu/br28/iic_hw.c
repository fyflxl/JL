...
#pragma bss_seg(".iic_hw.data.bss")
#pragma data_seg(".iic_hw.data")
#pragma const_seg(".iic_hw.text.const")
#pragma code_seg(".iic_hw.text")
#pragma str_literal_override(".iic_hw.text.const")
...
static u32 iic_baud[IIC_HW_NUM];
...
int hw_iic_set_baud(hw_iic_dev iic, u32 baud)
{
    ...
    //*3/2 by ic
    iic_baud_reg(iic_regs[id]) = sysclk / (2 * baud * 3 / 2) - 1;
    iic_baud[id] = baud;
    return 0;
}
...
static void iic_enter_deepsleep(void)
{
    IIC_CON0 = JL_IIC->CON0;
    IIC_CON1 = JL_IIC->CON1;
    /* IIC_BAUD = JL_IIC->BAUD;  */
    IIC_BAUD = clk_get("lsb") / (2 * iic_baud[0] * 3 / 2) - 1;
}

AT(.iic_hw.text.cache.L1)
void iic_exit_deepsleep(void)
{

    JL_IIC->CON0 = IIC_CON0;
    JL_IIC->CON1 = IIC_CON1;
    JL_IIC->BAUD = IIC_BAUD ;
}



DEEPSLEEP_TARGET_REGISTER(iic) = {
    .name   = "iic",
    .enter  = iic_enter_deepsleep,
    .exit   = iic_exit_deepsleep,
};
