...
struct a2dp_dec_hdl {
    ...
    void *bcsync ;//广播同步私有参数
    void (*format_sync)(void *bcsync, int sample_rate, u8 channel_num); //解码参数更新到同步接口
    void (*resume_sync)(void *bcsync, void *priv, void (*resume)(void *priv)); //同步阻塞激活接口
...
*/
/*----------------------------------------------------------------------------*/
void broadcast_a2dp_decoder_kick(void *priv)
{
...
static int a2dp_dec_pcm_capture_start(void)
{
    ...
    //更新广播同步配置参数
    dec->format_sync(dec->bcsync, fmt->sample_rate, dec->pcm_ch_num);
    dec->resume_sync(dec->bcsync, dec, broadcast_a2dp_decoder_kick);
...
static int a2dp_dec_start(void)
{
    ...
    vocal_remove_low_cut_samplerate(fmt->sample_rate);
#endif
...
int a2dp_dec_to_pcm_open(struct a2dp_dec_params *params)
{
    ...
    dec->bcsync = a2dp_dec_params->bcsync;
    dec->format_sync = a2dp_dec_params->format_sync;
    dec->resume_sync = a2dp_dec_params->resume_sync;
