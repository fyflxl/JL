...
struct file_dec_params {
    ...
    void (*format_sync)(void *priv, int sample_rate, u8 channel_num); //解码参数更新到同步
...
#if defined(TCFG_EQ_DIVIDE_ENABLE) && TCFG_EQ_DIVIDE_ENABLE&& !WIRELESS_SOUND_TRACK_2_P_X_ENABLE
    struct audio_eq *eq_rl_rr;    //eq drc句柄
    struct audio_drc *drc_rl_rr;    // drc句柄
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
    struct audio_drc *drc_rr;    // 右声道drc句柄
#endif
