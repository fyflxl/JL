...
#if 0//(AUDIO_OUT_WAY_TYPE & AUDIO_WAY_TYPE_BT)
...
#define AUDIO_CODEC_SUPPORT_SYNC	0 // 同步
...
void *data_viewer_last = NULL;
void *data_viewer_last_entry = NULL;
...
void last_data_viewer_callback(u8 ch, s16 *data, u32 len)
{
...
struct audio_eq *vocal_remove_eq = NULL;
#define VOCAL_REMOVE_LOW_CUT_FRE	180		//unit:Hz
struct eq_seg_info vocal_remove_fre_cut_eq_tab[] = {
    {0, EQ_IIR_TYPE_LOW_PASS,  VOCAL_REMOVE_LOW_CUT_FRE,    0, 0.7f},
    {1, EQ_IIR_TYPE_LOW_PASS,  VOCAL_REMOVE_LOW_CUT_FRE,    0, 0.7f},
    {2, EQ_IIR_TYPE_LOW_PASS,  VOCAL_REMOVE_LOW_CUT_FRE,    0, 0.7f},
};

int vocal_remove_low_cut_run(s16 *data, u16 len);
int vocal_remove_low_cut_open(u8 ch, u32 sample_rate)
{
    printf("vocal_remove_eq_open,ch:%d,sample_rate:%d\n", ch, sample_rate);

    struct audio_eq_param eq_param = {0};
    eq_param.sr = sample_rate;
    eq_param.channels = ch;
    eq_param.max_nsection = ARRAY_SIZE(vocal_remove_fre_cut_eq_tab);
    eq_param.nsection = ARRAY_SIZE(vocal_remove_fre_cut_eq_tab);
    eq_param.seg = vocal_remove_fre_cut_eq_tab;
    eq_param.global_gain = 0;
    eq_param.cb = eq_get_filter_info;
    printf("low_cut nSection:%d\n", eq_param.nsection);
    vocal_remove_eq = audio_dec_eq_open(&eq_param);
    mix_vocal_remove_hdl->low_cut_cb = vocal_remove_low_cut_run;
    return 0;
}

int vocal_remove_low_cut_close(void)
{
    if (vocal_remove_eq) {
        audio_dec_eq_close(vocal_remove_eq);
    }
    return 0;
}
...
int vocal_remove_low_cut_run(s16 *data, u16 len)
{
    if (vocal_remove_eq) {
        // putchar('e');
        audio_eq_run(vocal_remove_eq, data, len);
    }
    return 0;
}
void vocal_remove_low_cut_samplerate(u32 sr)
{
    if (vocal_remove_eq) {
        printf("vocal_remove_eq_set_samplerate:%d\n", sr);
        audio_eq_set_samplerate(vocal_remove_eq, sr);
    }
}
#endif
...
void *audio_decoder_malloc(int size)
{
...
void audio_decoder_free(void *p)
{
    if (check_ram1_size() == 0) {
        free(p);
    } else {
...
void *audio_encoder_malloc(int size)
{
    if (check_ram1_size() == 0) {
        return malloc(size);
    } else {
...
void audio_encoder_free(void *p)
{
    if (check_ram1_size() == 0) {
        free(p);
    } else {
...
int audio_decoder_wakeup_select(u8 source)
{
    if (source == 0) {
        ...
        local_irq_disable();
        if (!decode_task.wakeup_timer) {
            decode_task.wakeup_timer = usr_timeout_add(&decode_task, audio_decoder_wakeup_timer, AUDIO_DECODE_TASK_WAKEUP_TIME, 1);
        }
        local_irq_enable();
    } else if (source == 1) {
#if 0
...
#elif (AUDIO_OUT_WAY_TYPE & AUDIO_WAY_TYPE_FM)
    return 41667;
#else
    return 44100;
#endif
...
#elif (AUDIO_OUT_WAY_TYPE & AUDIO_WAY_TYPE_DONGLE)
...
u32 audio_output_channel_num(void)
{
    ...
    return  2;
#endif
    return app_audio_output_channel_get();//解码输出不支持4声道，如果是四声道，app_audio_output_channel_get需要特殊处理 return 2
}
...
u32 audio_output_channel_type(void)
{
#if (AUDIO_OUT_WAY_TYPE & AUDIO_WAY_TYPE_DAC)
    /*根据DAC输出的方式选择输出的声道*/
    u8 dac_connect_mode =  TCFG_AUDIO_DAC_CONNECT_MODE;
    ...
    } else if (dac_connect_mode == DAC_OUTPUT_MONO_LR_DIFF) {
        return AUDIO_CH_LR;
    } else {
...
#define MIXER_MIN_LEN		(128*4*2)
...
#if (TCFG_AUDIO_DAC_CONNECT_MODE == DAC_OUTPUT_MONO_LR_DIFF)
    data_viewer_last = audio_data_viewer_open(last_data_viewer_callback);
    data_viewer_last_entry = audio_data_viewer_entry_get(data_viewer_last);
#endif
    ...
    vocal_remove_low_cut_open(2, 44100);
    /* u8 dac_connect_mode =  app_audio_output_mode_get(); */
    u8 dac_connect_mode =  TCFG_AUDIO_DAC_CONNECT_MODE;
    if (dac_connect_mode == DAC_OUTPUT_MONO_L) {
        vocal_remove_mix_ch_switch = channel_switch_open(AUDIO_CH_L, 0);
    } else if (dac_connect_mode == DAC_OUTPUT_MONO_R) {
        vocal_remove_mix_ch_switch = channel_switch_open(AUDIO_CH_R, 0);
    }
#endif
...
#if (TCFG_AUDIO_DAC_CONNECT_MODE == DAC_OUTPUT_MONO_LR_DIFF)
    if (data_viewer_last_entry) {
        entries[entry_cnt++] = data_viewer_last_entry;
    }
#endif
    ...
    audio_app_stream_init();
    entries[entry_cnt++] = audio_app_stream_get_entry();
...
#if 0//(AUDIO_OUT_WAY_TYPE & AUDIO_WAY_TYPE_IIS)
    audio_dig_vol_param iis_digvol_last_param = {
        .vol_start = app_var.music_volume,
        .vol_max = SYS_MAX_VOL,
        .ch_total = 2,
...
*/
/*----------------------------------------------------------------------------*/
void vocal_remove_switch(u8 sw)
{
    if (mix_vocal_remove_hdl) {
