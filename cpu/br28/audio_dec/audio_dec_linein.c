...
int linein_dec_start()
{
...
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
    dec->drc_fr = linein_drc_fr_open(dec->pcm_dec.sample_rate, dec->pcm_dec.output_ch_num);//drc
#endif
#else
    ...
    dec->last_drc = aux_last_drc_open(dec->pcm_dec.sample_rate, dec->pcm_dec.output_ch_num);
#else
...
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
    dec->drc_rr = aux_drc_rr_open(dec->pcm_dec.sample_rate, dec->pcm_dec.output_ch_num);//drc
#endif
...
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
    linein_drc_fr_close(dec->drc_fr);
#endif
#else
...
static void __linein_dec_close(void)
{
    if (linein_dec && linein_dec->start) {
...
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
        linein_drc_fr_close(linein_dec->drc_fr);
#endif
#else
        music_drc_close(linein_dec->drc);
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
        music_drc_fr_close(linein_dec->drc_fr);
#endif
#endif
#endif
#if defined(TCFG_DRC_ENABLE) && TCFG_DRC_ENABLE
        ...
        music_last_drc_close(linein_dec->last_drc);
...
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
        music_drc_rr_close(linein_dec->drc_rr);
#endif
