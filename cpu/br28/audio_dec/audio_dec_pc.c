...
#if (TCFG_APP_PC_EN && (USB_DEVICE_CLASS_CONFIG & AUDIO_CLASS))
...
static int uac_audio_close(void)
{
...
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
    music_drc_fr_close(uac_dec->drc_fr);
#endif
#endif
#if defined(TCFG_DRC_ENABLE) && TCFG_DRC_ENABLE
    ...
    music_last_drc_close(uac_dec->last_drc);
...
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
    music_drc_rr_close(uac_dec->drc_rr);
#endif
