...
extern int broadcast_audio_sync_block_input(void *basync, void *data, int len);
extern void  *get_sync_hdl();
void *sync_data_buf;
static u16 mute_data_output_timer;

void mute_data_output_timer_del(void)
{
    if (mute_data_output_timer) {
        sys_timer_del(mute_data_output_timer);
        if (sync_data_buf) {
            free(sync_data_buf);
            sync_data_buf = NULL;
        }
        printf("enter audio_dec_file.c ,sys_timer_del%d\n", __LINE__);
        mute_data_output_timer = 0;
    }
}

void mute_data_output_timer_handle(void)
{
    void *output_priv = get_sync_hdl();
    u16 len = 1920;
    if (!sync_data_buf) {
        sync_data_buf = zalloc(len);
    }
    if (!sync_data_buf) {
        //printf("enter audio_dec_file.c timer%d\n",__LINE__);
        return;
    }
    if (output_priv) {
        int wlen = broadcast_audio_sync_block_input(output_priv, sync_data_buf, len); //输出几次0
        //printf("enter audio_dec_file.c timer%d,%d\n",__LINE__,wlen);
    } else {
        if (app_get_curr_task() != APP_MUSIC_TASK) {
            //printf("t:%d\n",__LINE__);
            mute_data_output_timer_del();
        }
    }
}
...
void set_file_dec_start_pause(u8 file_dec_initial)
{
    ...
    file_dec_start_pause = file_dec_initial;
}
/*----------------------------------------------------------------------------*/
/**@brief    获取文件解码hdl
   @param
...
int file_decoder_stop_handler_app(struct audio_decoder *decoder)	// 解码结束
{
#if TCFG_BROADCAST_ENABLE
    struct file_decoder *dec = container_of(decoder, struct file_decoder, decoder);
    if (!mute_data_output_timer) {
        //printf("enter audio_dec_file.c ,sys_timer_add%d\n",__LINE__);
        if (app_get_curr_task() == APP_MUSIC_TASK) {
            mute_data_output_timer =  sys_timer_add(NULL, mute_data_output_timer_handle, 10);
        }
    }
#endif
    return 0;
...
static const struct audio_dec_handler file_decoder_handler_app = {
    ...
    .dec_stop  = file_decoder_stop_handler_app,
};
#endif
...
*/
/*----------------------------------------------------------------------------*/
void broadcast_file_decoder_kick(void *priv)
{
    if (!file_dec) {
        return ;
    }
    if (file_dec->file_dec.status) {
        audio_decoder_resume(&file_dec->file_dec.decoder);
    }
}
/*----------------------------------------------------------------------------*/
/**@brief   解码输出pcm数据流节点
...
*/
/*----------------------------------------------------------------------------*/
struct file_dec_to_pcm_hdl {
    struct audio_stream_entry entry;
} file_to_pcm_hdl;

static void file_dec_to_pcm_process_len(struct audio_stream_entry *entry,  int len)
{
}
// 解码输出数据流节点数据处理回调
static int file_dec_to_pcm_data_handler(struct audio_stream_entry *entry,
                                        struct audio_data_frame *in,
                                        struct audio_data_frame *out)
{
    struct file_dec_to_pcm_hdl *hdl = container_of(entry, struct file_dec_to_pcm_hdl, entry);
    int wlen;
    /* putchar('D'); */
    wlen = file_dec->output_handler(file_dec->output_priv, in->data, in->data_len);
...
static int file_dec_start(void)
{
    ...
    //更新参数到同步
    if (dec->pcm_capture) {
        audio_decoder_set_handler(&dec->file_dec.decoder, &file_decoder_handler_app);
        dec->format_sync(dec->bcsync, dec->file_dec.sample_rate, dec->file_dec.output_ch_num);
        dec->resume_sync(dec->bcsync, dec, broadcast_file_decoder_kick);
    }
__open_ok:
    if (!dec->pcm_capture) {
#if CHECK_SR_WHEN_DECODING
    ...
    dec->drc_fr = music_drc_fr_open(dec->file_dec.sample_rate, dec->file_dec.output_ch_num);//drc
#endif
    ...
    dec->last_drc = music_last_drc_open(dec->file_dec.sample_rate, dec->file_dec.output_ch_num);
...
#if TCFG_EQ_ENABLE && TCFG_MUSIC_MODE_EQ_ENABLE
    dec->eq_rl_rr = music_eq_rl_rr_open(dec->file_dec.sample_rate, dec->file_dec.output_ch_num);// eq
#if TCFG_DRC_ENABLE &&TCFG_MUSIC_MODE_DRC_ENABLE
    dec->drc_rl_rr = music_drc_rl_rr_open(dec->file_dec.sample_rate, dec->file_dec.output_ch_num);//drc
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
    dec->drc_rr = music_drc_rr_open(dec->file_dec.sample_rate, dec->file_dec.output_ch_num);//drc
#endif
#endif
...
#if defined(TCFG_PHASER_GAIN_AND_CH_SWAP_ENABLE) && TCFG_PHASER_GAIN_AND_CH_SWAP_ENABLE
    dec->gain_rl_rr = audio_gain_open_demo(AEID_MUSIC_RL_GAIN, dec->file_dec.output_ch_num);
#endif
    ...
    dec->ext_eq2 = music_ext_eq2_open(dec->file_dec.sample_rate, dec->file_dec.output_ch_num);
#endif
    ...
    //添加数据流节点，解码从这里出pcm数据，该节点不向后续节点传递数据
    if (dec->pcm_capture) {
        file_to_pcm_hdl.entry.data_process_len = file_dec_to_pcm_process_len;
        file_to_pcm_hdl.entry.data_handler = file_dec_to_pcm_data_handler;
        entries[entry_cnt++] = &(file_to_pcm_hdl.entry);
...
#endif
    if (!dec->pcm_capture) { //解码输出pcm数据的流程不需要设置音量
        // 设置音频输出音量
...
int file_dec_open(void *file, struct audio_dec_breakpoint *bp)
{
    ...
    //如果解码只需要pcm数据 根据传进来的声道数配置pcm输出的声道数
    if (dec->pcm_capture) {
        dec->file_dec.output_ch_num = dec->pcm_ch_num;
        dec->file_dec.output_ch_type = dec->pcm_ch_num == 2 ? AUDIO_CH_LR :	AUDIO_CH_DIFF;
    }
    ...
    if (file_dec->pcm_capture) {
        dec->wait.protect = 1;  //获取pcm数据时，解码处于后台，不能被打断
    } else {
        dec->wait.protect = 0;
    }
...
*/
/*----------------------------------------------------------------------------*/
int file_dec_get_fmt(int *sr, u8 *ch_num)
{
    if (!file_dec) {
        return 1;
    }
    ...
    *sr = file_dec->file_dec.sample_rate;
    *ch_num = file_dec->file_dec.ch_num;
    return 0;
}
/*----------------------------------------------------------------------------*/
/**@brief    打开文件解码 只解出pcm数据 不向后级输出 通过参数的输出接口输出pcm数据
   @param    *params:文件解码参数
   @return   0：成功
   @return   非0：失败
   @note
*/
/*----------------------------------------------------------------------------*/
int file_dec_to_pcm_open(struct file_dec_params *params)
{
    struct file_dec_params *file_dec_params = params;
    if (!file_dec_params) {
        printf("file_dec params  err !!!\n");
        return 1;
    }
    file_dec->output_priv = file_dec_params->output_priv;
    file_dec->output_handler = file_dec_params->output_handler;
    file_dec->bcsync = file_dec_params->bcsync;
    file_dec->format_sync = file_dec_params->format_sync;
    file_dec->resume_sync = file_dec_params->resume_sync;
    file_dec->pcm_ch_num = file_dec_params->ch_num;

    file_dec->pcm_capture = 1;
    return file_dec_open(file_dec_params->file, file_dec_params->bp);
}
...
void file_dec_close(void)
{
    ...
    if (file_dec->file_dec.status) {
...
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
        music_drc_fr_close(file_dec->drc_fr);
#endif
#endif
#if defined(TCFG_DRC_ENABLE) && TCFG_DRC_ENABLE
        ...
        music_last_drc_close(file_dec->last_drc);
...
#if TCFG_EQ_DIVIDE_ENABLE&& !WIRELESS_SOUND_TRACK_2_P_X_ENABLE
        music_eq_rl_rr_close(file_dec->eq_rl_rr);
        music_drc_rl_rr_close(file_dec->drc_rl_rr);
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
        music_drc_rr_close(file_dec->drc_rr);
#endif
        convet_data_close(file_dec->convert_rl_rr);
#if defined(TCFG_PHASER_GAIN_AND_CH_SWAP_ENABLE) && TCFG_PHASER_GAIN_AND_CH_SWAP_ENABLE
        audio_gain_close_demo(file_dec->gain_rl_rr);
#endif
...
#if defined(MUSIC_EXT_EQ2_AFTER_DRC) && MUSIC_EXT_EQ2_AFTER_DRC
        music_ext_eq2_close(file_dec->ext_eq2);
#endif
#endif
...
int file_dec_by_tmark(void *breakpoint, u16 size)
{
    ...
    if (breakpoint && file_dec->file_dec.status && file_dec->bp) {
        put_buf(breakpoint, size);
        ASSERT(size <=  BREAKPOINT_DATA_LEN);
        file_dec->bp->len = size;
        memcpy(file_dec->bp->data, breakpoint, size);
    }
    void *bp = file_dec->bp;
...
void file_decoder_pp_ctrl(struct file_decoder *dec, u8 play)
{
    ...
    if (file_dec && (&file_dec->file_dec == dec)) {
        ...
        audio_stream_clear(file_dec->stream);
    }
#endif /*TCFG_APP_MUSIC_EN*/
}
