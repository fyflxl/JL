...
static void *audio_track = NULL;
#define  SPDIF_SRC_EN  1
int spdif_stream_sample_rate(void *hdl);
...
int spdif_dec_write_data(void *data, int len)
{
    ...
    if (audio_track) {
        audio_local_sample_track_in_period(audio_track, (len / 2 / 2));
    }
...
static int spdif_dec_read(void *hdl, void *buf, int len)
{
    ...
    if (cbuf_get_data_size(&spdif_dec_hdl->dec_cbuf) >= SPDIF_DATA_DAM_LEN * SPDIF_CHANNEL_NUMBER * 2 * 2) {
        rlen = cbuf_read(&spdif_dec_hdl->dec_cbuf, buf, SPDIF_DATA_DAM_LEN * SPDIF_CHANNEL_NUMBER * 2);
    }
    /* printf("rlen:%d\n",rlen); */
    return rlen;
}
                                ...
                                struct audio_data_frame *out)
{
    ...
    if (in->offset == 0) {
#if SPDIF_SRC_EN
        in->sample_rate = spdif_stream_sample_rate(NULL);
#else
...
{
    if (audio_track) {
        int sr =  audio_local_sample_track_rate(audio_track);
        /* printf("%d\n",sr); */
        if ((sr < (spdif_dec_hdl->pcm_sr + 200)) && (sr > spdif_dec_hdl->pcm_sr - 200)) {
            return sr;
        }
        printf("spdif audio_track reset \n");
        local_irq_disable();
        audio_local_sample_track_close(audio_track);
        audio_track = audio_local_sample_track_open(2, spdif_dec_hdl->pcm_sr, 1000);
        local_irq_enable();
    }
    return spdif_dec_hdl->pcm_sr;
}

static int audio_spdif_input_sample_rate(void *priv)
{

    int sample_rate_compare = spdif_stream_sample_rate(NULL);
    int sample_rate = spdif_stream_sample_rate(NULL);
    int buf_size = cbuf_get_data_size(&spdif_dec_hdl->dec_cbuf);
    /* if (dec->pcm_dec.sample_rate == 44100 && audio_mixer_get_sample_rate(&mixer) == 44100) { */
    if ((spdif_dec_hdl->pcm_sr == 44100) && (audio_mixer_get_original_sample_rate_by_type(&mixer, MIXER_SR_SPEC) == 44100)) {
        sample_rate = 44100 + (sample_rate - 44118);
    }
#if 0
    if (buf_size >= (SPDIF_DEC_DATA_BUF_LEN * 3 / 10)) {
        sample_rate += (sample_rate * 6 / 10000);
    }
    if (buf_size <= (SPDIF_DEC_DATA_BUF_LEN * 1 / 10)) {
        sample_rate -= (sample_rate * 6 / 10000);
    }
#endif
    /* printf("%d,%d,%d\n",sample_rate,buf_size,sample_rate_compare); */
    return sample_rate;
}
...
int spdif_dec_start(void)
{
    ...
    audio_mixer_ch_set_sample_rate(&dec->mix_ch, dec->fmt.sample_rate);
#if SPDIF_SRC_EN
    dec->src = stream_src_open(dec->fmt.sample_rate, 1);
    audio_mixer_ch_set_src(&dec->mix_ch, 0, 0);
#else
    audio_mixer_ch_follow_resample_enable(&dec->mix_ch, dec, audio_spdif_input_sample_rate);
...
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
    dec->drc_fr = music_drc_fr_open(sample_rate, ch_num);//drc
#endif
    ...
    dec->last_drc = music_last_drc_open(sample_rate, ch_num);
...
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
    dec->drc_rr = music_drc_rr_open(sample_rate, ch_num);//drc
#endif
#endif
    ...
    dec->ext_eq2 = music_ext_eq2_open(sample_rate, ch_num);
#endif
...
#if SPDIF_SRC_EN
    if (dec->src) {
        entries[entry_cnt++] = &dec->src->entry;
    }
#endif
...
int spdif_dec_stop(void)
{
    if (spdif_dec_hdl) {
        ...
        if (dec->audio_stream) {
            audio_stream_close(dec->audio_stream);
            dec->audio_stream = 0;
        }
    }
    return 0;
...
#endif
    spdif_slave_hdl.input_port  = SPDIF_IN_PORT_C;//PB0//SPDIF_IN_PORT_A;//PA9
    audio_spdif_slave_open(&spdif_slave_hdl);
