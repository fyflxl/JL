...
#if defined(AUDIO_PGA_CONFIG)&&AUDIO_PGA_CONFIG
    gain_l  = gain_l > dac_pga.again_fl ? dac_pga.again_fl : gain_l;
    gain_r  = gain_r > dac_pga.again_fr ? dac_pga.again_fr : gain_r;
#endif
...
#if defined(AUDIO_PGA_CONFIG)&&AUDIO_PGA_CONFIG
    target_again_l  = target_again_l > dac_pga.again_fl ? dac_pga.again_fl : target_again_l;
    target_again_r  = target_again_r > dac_pga.again_fr ? dac_pga.again_fr : target_again_r;
    target_dgain_l = target_dgain_l > dac_pga.dgain_fl ? dac_pga.dgain_fl : target_dgain_l;
    target_dgain_r = target_dgain_r > dac_pga.dgain_fr ? dac_pga.dgain_fr : target_dgain_r;
#endif
...
void audio_fade_in_fade_out(u8 left_gain, u8 right_gain, u8 fade)
{
    ...
    u8 dvol_hw_level, dvol_hw_level_r = 0;
    dvol_hw_level = left_gain * DVOL_HW_LEVEL_MAX / get_max_sys_vol();
    dvol_hw_level_r = right_gain * DVOL_HW_LEVEL_MAX / get_max_sys_vol();
    __this->digital_volume = hw_dig_vol_table[dvol_hw_level];
    __this->digital_volume_r = hw_dig_vol_table[dvol_hw_level_r];

    audio_way_set_analog_gain(AUDIO_WAY_TYPE_DAC, SOUND_CHMAP_FL | SOUND_CHMAP_FR, analog_vol);
    audio_way_set_digital_gain(AUDIO_WAY_TYPE_DAC, SOUND_CHMAP_FL, __this->digital_volume);
    audio_way_set_digital_gain(AUDIO_WAY_TYPE_DAC, SOUND_CHMAP_FR, __this->digital_volume_r);
    printf(">>> A:%d DL:%d DR:%d\n", analog_vol, __this->digital_volume, __this->digital_volume_r);
#else
...
#if (SMART_BOX_EN && RCSP_ADV_EQ_SET_ENABLE)
    extern bool smartbox_set_volume(s8 volume);
    if (smartbox_set_volume(volumei_l)) {
        return;
    }
#endif
#if (RCSP_MODE && RCSP_ADV_EQ_SET_ENABLE)
    //extern bool rcsp_set_volume(s8 volume);
    if (rcsp_set_volume(volume_l)) {
        return;
    }
#endif
        ...
        if (SYS_VOL_TYPE == VOL_TYPE_DIGGROUP && SYS_DIGVOL_GROUP_EN) {
            ...
            if (strcmp(digvol_type, "music_type") == 0) {
                for (int i = 0; music_dig_logo[i] != "NULL"; i++) {
                    ...
                    audio_dig_vol_group_vol_set(sys_digvol_group, music_dig_logo[i], AUDIO_DIG_VOL_CH(0), volume_l);
                    audio_dig_vol_group_vol_set(sys_digvol_group, music_dig_logo[i], AUDIO_DIG_VOL_CH(1), volume_r);
                    /* audio_dig_vol_group_vol_set(sys_digvol_group, music_dig_logo[i], AUDIO_DIG_VOL_ALL_CH, volume); */
                }
            } else {
                if (audio_dig_vol_group_hdl_get(sys_digvol_group, digvol_type) == NULL) {
                ...
                audio_dig_vol_group_vol_set(sys_digvol_group, digvol_type, AUDIO_DIG_VOL_CH(0), volume_l);
                audio_dig_vol_group_vol_set(sys_digvol_group, digvol_type, AUDIO_DIG_VOL_CH(1), volume_r);
                /* audio_dig_vol_group_vol_set(sys_digvol_group, digvol_type, AUDIO_DIG_VOL_ALL_CH, volume); */
            }
...
s8 app_audio_get_volume_r(u8 state)
{
    ...
    case APP_AUDIO_STATE_MUSIC:
    case APP_AUDIO_STATE_LINEIN:
        volume = app_var.music_volume_r;
        break;
    case APP_AUDIO_STATE_CALL:
        volume = app_var.call_volume_r;
        break;
    case APP_AUDIO_STATE_WTONE:
#if TONE_MODE_DEFAULE_VOLUME != 0
        app_var.wtone_volume = TONE_MODE_DEFAULE_VOLUME;
...
#endif
    case APP_AUDIO_CURRENT_STATE:
        volume = app_audio_get_volume_r(__this->state);
        break;
    default:
        break;
    }
...
void app_audio_volume_down(u8 value)
{
    ...
    if (samrtbox_key_volume_down(value)) {
        return;
    }
#endif
#if (RCSP_MODE && RCSP_ADV_EQ_SET_ENABLE)
...
void app_audio_state_switch(u8 state, s16 max_volume)
{
    ...
    audio_way_set_analog_gain(AUDIO_WAY_TYPE_DAC, SOUND_CHMAP_FL | SOUND_CHMAP_FR, 3);
    audio_way_set_digital_gain(AUDIO_WAY_TYPE_DAC, SOUND_CHMAP_FL, 16384);
    audio_way_set_digital_gain(AUDIO_WAY_TYPE_DAC, SOUND_CHMAP_FR, 16384);
#endif
