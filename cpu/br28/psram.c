...
void psram_hw_enter_sleep(void)
{
    ...
    if ((__this->init == 0) || (__this->low_power == 1)) {
        return;
    }
...
static u8 psram_enter_deepsleep(void)
{
    if (__this->init) {
...
static u8 psram_exit_deepsleep(void)
{
    if ((__this->init == 0) || (__this->low_power == 1)) {
        return 0;
    }
...
DEEPSLEEP_TARGET_REGISTER(psram_deep_sleep) = {
    .name   = "psram",
    .enter  = psram_enter_deepsleep,
    .exit   = psram_exit_deepsleep,
};
