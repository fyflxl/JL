...
#define SBC_AUDIO_OVERLAY		1	// sbc发射和audio空间互用

#define BT_USE_PCM_DEC         0   //多加一个解码，会多耗资源，时钟也会增加,用来解决完全切歌的时候出现杂音的问题

#define  SBC_RESERVED_LAST_PCM  0  //淡入淡出功能，耗资源很小，可以解决正常速度切歌杂音，但是快速连续切歌会有杂音
...
struct sbc_enc_hdl {
    ...
    u8  pcm_frame[SBC_ENC_IN_SIZE * 2];
    u8 frame_size;
...
extern void *audio_dac_get_buf(int *len);
...
static int sbc_enc_pcm_get(struct audio_encoder *encoder, s16 **frame, u16 frame_len)
{
    ...
    if (read_flag) {
        if (audio_output_channel_num() != audio_sbc_enc_get_channel_num()) {
            /* putchar('S'); */
            if (audio_output_channel_num() == 1) {
                pcm_len = cbuf_read(&enc->pcm_in_cbuf, (void *)((int)enc->pcm_frame + (SBC_ENC_IN_SIZE / 2)), SBC_ENC_IN_SIZE / 2);
                if (pcm_len) {
                    audio_pcm_mono_to_dual(enc->pcm_frame, (void *)((int)enc->pcm_frame + (SBC_ENC_IN_SIZE / 2)), pcm_len / 2);
                    pcm_len <<= 1;
                }
            } else {
                pcm_len = cbuf_read(&enc->pcm_in_cbuf, enc->pcm_frame, SBC_ENC_IN_SIZE * 2);
                if (pcm_len) {
                    audio_decoder_dual_switch(AUDIO_CH_DIFF, 1, enc->pcm_frame, pcm_len);
                    pcm_len >>= 1;
                }
            }
        } else {
...
{
#if (AUDIO_OUT_WAY_TYPE & AUDIO_WAY_TYPE_BT)
    extern void audio_bt_emitter_hw_output(void);
...
void audio_sbc_enc_data_start_pre(void)
{
    if (!sbc_enc) {
        return ;
    }
...
void audio_sbc_enc_data_stop_pre(void)
{
    if (!sbc_enc) {
        return ;
    }
...
int audio_sbc_enc_write_alloc(s16 **buf, u32 *len)
{
    if (!(sbc_buf_flag & SBC_ENC_START)) {
        return false;
    }
    ...
    audio_sbc_enc_data_start_pre();

    sbc_buf_flag |= SBC_ENC_WRITING;

    u32 wlen = 0;
    *buf = cbuf_write_alloc(&sbc_enc->pcm_in_cbuf, &wlen);
    *len = wlen;

    return true;
}

int audio_sbc_enc_write_updata(u32 len)
{
    if (!(sbc_buf_flag & SBC_ENC_START)) {
        return false;
    }
    ...
    cbuf_write_updata(&sbc_enc->pcm_in_cbuf, len);
...
void audio_sbc_enc_resume(void)
{
    if (!(sbc_buf_flag & SBC_ENC_START)) {
        return ;
    }
...
int audio_sbc_enc_open()
{
    ...
    u16 frame_len = 512;
    u16 in_size = SBC_ENC_IN_CBUF_SIZE;
    u16 out_size = SBC_ENC_OUT_CBUF_SIZE;
    ...
    if (!sbc_enc) {
        ...
        int len = 0;
        cbuf_buf = audio_dac_get_buf(&len);
        if (cbuf_buf && (len >= (out_size + in_size))) {
            sbc_enc->overlay = 1;
            out_size = ((len - in_size) / SBC_ENC_OUT_SIZE) * SBC_ENC_OUT_SIZE;
            in_size = len - out_size;
            /* audio_way_close(AUDIO_WAY_TYPE_DAC); */
        } else
#endif
        {
            cbuf_buf = malloc(out_size + in_size);
        }
        sbc_enc->p_out_cbuf_buf = cbuf_buf;
        sbc_enc->p_in_cbuf_buf = &cbuf_buf[out_size];
    }
    printf("init vol:%d \n", app_var.music_volume);
    audio_way_set_gain(AUDIO_WAY_TYPE_BT, app_var.music_volume);
    cbuf_init(&sbc_enc->output_cbuf, sbc_enc->p_out_cbuf_buf, out_size);
    cbuf_init(&sbc_enc->pcm_in_cbuf, sbc_enc->p_in_cbuf_buf, in_size);
    ...
    audio_adc_mic_open(&sbc_enc->mic_ch, TCFG_AUDIO_ADC_MIC_CHA, &adc_hdl);
    ...
    /* audio_way_open(AUDIO_WAY_TYPE_BT); */
...
int audio_sbc_enc_close()
{
    ...
    /* audio_way_close(AUDIO_WAY_TYPE_BT); */
...
void bt_emitter_set_vol(u8 vol)
{
    ...
    r_printf("=======set vol %d\n", vol);
#if (AUDIO_OUT_WAY_TYPE & AUDIO_WAY_TYPE_BT)
    audio_way_set_gain(AUDIO_WAY_TYPE_BT, vol);
#endif
}
...
int a2dp_sbc_encoder_init(void *sbc_struct)
{
    ...
    if (sbc_struct) {
        ...
        /* audio_mixer_set_channel_num(&mixer, audio_sbc_enc_get_channel_num()); */
        audio_app_stream_switch_way(AUDIO_WAY_TYPE_DAC, AUDIO_WAY_TYPE_BT);
        //audio_sbc_enc_config(&sbc_param);
        audio_sbc_enc_open();
    } else {
        g_printf("sbc enc close");
        audio_mixer_reset_sample_rate(0, audio_sbc_enc_get_rate());
        /* audio_mixer_set_channel_num(&mixer, audio_output_channel_num()); */
        audio_app_stream_switch_way(AUDIO_WAY_TYPE_BT, AUDIO_WAY_TYPE_DAC);
        audio_sbc_enc_close();
    }
    return 0;
}
