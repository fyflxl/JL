...
#if TCFG_DYNAMIC_EQ_ENABLE
    {AEID_LAST_WHOLE_DRC, 0, 1,    {EFF_MUSIC_LAST_WHOLE_DRC}},
#endif
};
...
#if TCFG_DYNAMIC_EQ_ENABLE
    {AEID_AUX_LAST_WHOLE_DRC, 0, 1,    {EFF_MUSIC_LAST_WHOLE_DRC}},
#endif
};
#endif
...
#endif
    //更新参数到相应模块
#if defined (VOL_TYPE_DIGGROUP) && (SYS_DIGVOL_GROUP_EN)
    s8 volume = get_max_sys_vol();
    audio_fade_in_fade_out(volume, volume, 0);
#else
...
int last_whole_wdrc_parm_analyze(EFF_ONLINE_PACKET *packet, u8 id, u8 sq)
{
#if TCFG_DYNAMIC_EQ_ENABLE
    u16 type = 0; //0:whole drc
    u8 mode_id = packet->data[0];
    u8 tar = 0;
    u32 drc_name = get_module_name_and_index(packet->cmd, &type, tar);
    u8 update = 1;
    ...
    log_debug("wdrc_parm type %d\n", type);
    tar_wdrc = &last_whole_drc_parm[tar];
    memcpy(tar_wdrc, wdrc_parm, sizeof(wdrc_struct_TOOL_SET));
    wdrc_printf(tar_wdrc);
    if (update) {
        cur_drc_set_update(drc_name, type, tar_wdrc);
...
int ext_eff_file_analyze(u32 mode_id, u16 group_id, u8 tar_snap, void *data_buf, u32 buf_len)
{
    ...
    if (2 != fread(file, &ext_head_len, 2)) {
        ret = -9;
        goto err_exit;
    }

    u16 CNT = 0;//有多少个组需要额外的快照
    if (2 != fread(file, &CNT, 2)) {
        ret = -10;
        goto err_exit;
    }
    /* --------------------------------- */
    ...
    for (group_cnt = 0; group_cnt < CNT; group_cnt++) {
        ...
        if (2 != fread(file, &group, 2)) {
            ret = -12;
            goto err_exit;
        }
        if (2 != fread(file, &offset, 2)) {
            ret = -13;
            goto err_exit;
        }
        if ((mode_seq == mode_id) && (group_id == group)) {
            printf("--------------------group_id snap ok %x \n", group);
            break;
        }
    }
    if (group_cnt == CNT) {
    ...
    if (1 != fread(file, &snap_group_num, 1)) {
        ret = -14;
        goto err_exit;
    }
    /* printf("snap_group_num %x\n", snap_group_num); */
    ...
    for (snap_cnt = 0; snap_cnt < snap_group_num; snap_cnt++) {
        if (2 != fread(file, &snap_group_offset, 2)) {
            ret = -16;
            goto err_exit;
        }
    ...
    /* printf("snap_group_offset %x\n", snap_group_offset); */
    u16 snap_group_len = 0;
    if (2 != fread(file, &snap_group_len, 2)) {
        ret = -18;
        goto err_exit;
    }
    /* printf("gourp_len %x\n", snap_group_len); */
    if (snap_group_len <= buf_len) {
        fread(file, data_buf, snap_group_len);
    } else {
        ret = -19;
...
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
REGISTER_CMD_TARGET(ml_fr_drc) = {//music_drc
#if TCFG_EFFECT_TOOL_ENABLE
...
#if defined(MULTI_BAND_DRC) && MULTI_BAND_DRC
REGISTER_CMD_TARGET(mm_fr_drc) = {//music_drc
#if TCFG_EFFECT_TOOL_ENABLE
...
REGISTER_CMD_TARGET(mh_fr_drc) = {//music_drc
#if TCFG_EFFECT_TOOL_ENABLE
...
REGISTER_CMD_TARGET(m_fr_whole_drc) = {//music_drc
#if TCFG_EFFECT_TOOL_ENABLE
...
REGISTER_CMD_TARGET(m_fr_cross) = {//music_crossover
#if TCFG_EFFECT_TOOL_ENABLE
...
#if defined(MULTI_BAND_DRC) && MULTI_BAND_DRC
REGISTER_CMD_TARGET(mm_rl_drc) = {//music_drc
#if TCFG_EFFECT_TOOL_ENABLE
...
REGISTER_CMD_TARGET(mh_rl_drc) = {//music_drc
#if TCFG_EFFECT_TOOL_ENABLE
...
REGISTER_CMD_TARGET(m_rl_whole_drc) = {//music_drc
#if TCFG_EFFECT_TOOL_ENABLE
...
REGISTER_CMD_TARGET(m_rl_cross) = {//music_crossover
#if TCFG_EFFECT_TOOL_ENABLE
...
#endif
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
REGISTER_CMD_TARGET(rr_drc_p) = {//music_rl_drc
#if TCFG_EFFECT_TOOL_ENABLE
...
#if defined(MULTI_BAND_DRC) && MULTI_BAND_DRC
REGISTER_CMD_TARGET(mm_rr_drc) = {//music_drc
#if TCFG_EFFECT_TOOL_ENABLE
...
REGISTER_CMD_TARGET(mh_rr_drc) = {//music_drc
#if TCFG_EFFECT_TOOL_ENABLE
...
REGISTER_CMD_TARGET(m_rr_whole_drc) = {//music_drc
#if TCFG_EFFECT_TOOL_ENABLE
...
REGISTER_CMD_TARGET(m_rr_cross) = {//music_crossover
#if TCFG_EFFECT_TOOL_ENABLE
...
REGISTER_CMD_TARGET(last_wh_drc) = {
#if TCFG_EFFECT_TOOL_ENABLE
    .cmd = EFF_MUSIC_LAST_WHOLE_DRC,
    .cmd_deal = last_whole_wdrc_parm_analyze,
#endif
    .eff_default_parm_init = last_whole_wdrc_parm_default_init,
    .eff_file_analyze_init = last_whole_wdrc_file_analyze_init,
...
#if defined(MUSIC_EXT_EQ_SPILT)&&MUSIC_EXT_EQ_SPILT
REGISTER_CMD_TARGET(music_fr_ext_eq) = {//music ext eq
#if TCFG_EFFECT_TOOL_ENABLE
...
#if TCFG_DYNAMIC_EQ_ENABLE
    cur_drc_set_update(AEID_LAST_WHOLE_DRC, 0, NULL);//wdrc
#endif
...
#if TCFG_DYNAMIC_EQ_ENABLE
    cur_drc_set_update(AEID_AUX_LAST_WHOLE_DRC, 0, NULL);//wdrc
#endif
...
{
    //更换效果文件名
    memcpy((void *)EFF_CFG_FILE_NAME, (const void *)path, strlen((const char *)path));
    //重新解析效果文件
    eff_file_parm_init();
...
REGISTER_SNAP_TARGET(last_wh_drc_snap) = {
    .eff_snap_parm_switch  = last_whole_wdrc_snap_parm_switch,
};
#endif
 ...
 * */
void eff_snap_parm_switch(u8 tar_snap)
{
