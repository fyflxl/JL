...
#define ESCO_DL_LIMITER_THR	                0.0f //-60 ~0db  通话下行限幅器输出最大幅度（无配置文件时有效）
/*
 *混响音效默认eq系数
 * */
...
u16 music_mode_seq[] = {music_mode_seq0, linein_mode_seq};//音乐模式的标号
...
void last_whole_wdrc_parm_default_init()
{
#if defined(TCFG_DRC_ENABLE)&&TCFG_DRC_ENABLE
#if TCFG_DYNAMIC_EQ_ENABLE
    /* u16 index = 0; */
    u8 calc = mode_add;
    /* u16 group_id = EFF_MUSIC_LAST_WHOLE_DRC; */
    /* u32 drc_name = get_module_name_and_index(group_id, &index, nor_list_label); */
    /* u16 i = index; */
    struct threshold_group group[] = {{0, 0}, {50, 50}, {90, 90}};
    for (int tar = 0; tar < calc; tar++) {
        last_whole_drc_parm[tar].is_bypass = 0;
        last_whole_drc_parm[tar].parm.attacktime = 10;
        last_whole_drc_parm[tar].parm.releasetime = 300;
        last_whole_drc_parm[tar].parm.inputgain = 0;
        last_whole_drc_parm[tar].parm.outputgain = 0;
        last_whole_drc_parm[tar].parm.threshold_num = ARRAY_SIZE(group);
        memcpy(last_whole_drc_parm[tar].parm.threshold, group, sizeof(group));
        last_whole_drc_parm[tar].parm.rms_time = 25;
        last_whole_drc_parm[tar].parm.algorithm = 0;
        last_whole_drc_parm[tar].parm.mode = 1;
    }
#endif
#endif
}

void last_whole_wdrc_file_analyze_init()
{
#if defined(TCFG_DRC_ENABLE)&&TCFG_DRC_ENABLE
#if TCFG_DYNAMIC_EQ_ENABLE
    /* u16 index = 0; */
    ...
    /* u16 i = index; */
    for (int tar = 0; tar < calc; tar++) {
        void *tar_buf = &last_whole_drc_parm[tar];
        u16 tar_len = sizeof(last_whole_drc_parm[tar]);
        eff_file_analyze(music_mode_seq[tar], group_id, tar_buf, tar_len);
        wdrc_printf(tar_buf);
    }
#endif
...
void downlink_wideband_wdrc_parm_default_init()
{
    ...
    //通话下行drc
    struct threshold_group group[] = {{0, 0}, {90 + ESCO_DL_LIMITER_THR, 90 + ESCO_DL_LIMITER_THR}, {90.1f, 90 + ESCO_DL_LIMITER_THR}};
...
/*
 *music、aux的tunning_eq配置参数默认值初始化
 * */
void tunning_parm_default_init()
{
#if defined(TCFG_EQ_ENABLE) && TCFG_EQ_ENABLE && TCFG_AUDIO_OUT_EQ_ENABLE &&BASS_AND_TREBLE_ENABLE
    u8 calc = mode_add;
    for (int tar = 0; tar < calc; tar++) {
        //low
        tunning_eq[tar].seg[0].filt_type = EQ_IIR_TYPE_BAND_PASS;
        tunning_eq[tar].seg[0].center_freq = 100;
        tunning_eq[tar].seg[0].q = 0.7f;
        tunning_eq[tar].seg[0].min_gain = -12;
        tunning_eq[tar].seg[0].max_gain = 12;
        //mid
        tunning_eq[tar].seg[1].filt_type = EQ_IIR_TYPE_BAND_PASS;
        tunning_eq[tar].seg[1].center_freq = 1000;
        tunning_eq[tar].seg[1].q = 0.7f;
        tunning_eq[tar].seg[1].min_gain = -12;
        tunning_eq[tar].seg[1].max_gain = 12;
        //high
        tunning_eq[tar].seg[2].filt_type = EQ_IIR_TYPE_BAND_PASS;
        tunning_eq[tar].seg[2].center_freq = 12000;
        tunning_eq[tar].seg[2].q = 0.7f;
        tunning_eq[tar].seg[2].min_gain = -12;
        tunning_eq[tar].seg[2].max_gain = 12;
    }
#endif
}
/*
 *music、aux的tunning_eq效果文件配置参数获取
 * */
void tunning_file_analyze_init()
{
#if defined(TCFG_EQ_ENABLE) && TCFG_EQ_ENABLE && TCFG_AUDIO_OUT_EQ_ENABLE &&BASS_AND_TREBLE_ENABLE
    u8 calc = mode_add;
    for (int index = 0 ; index < calc; index++) {
        u16 group_id = EFF_MUSIC_TUNNING_EQ;
        void *tar_buf = (void *)&tunning_eq[index];
        u16 tar_len = sizeof(struct tunning_eq_tool);
        eff_file_analyze(music_mode_seq[index], group_id, tar_buf, tar_len);

#if PARM_DEBUG
        struct tunning_eq_tool *_parm = &tunning_eq[index];
        for (int i = 0; i < 3; i++) {
...
/*
 *混响mic的tunning_eq配置参数默认值初始化
 * */
void mic_tunning_parm_default_init()
{
#if defined(TCFG_MIC_EFFECT_ENABLE) && TCFG_MIC_EFFECT_ENABLE && BASS_AND_TREBLE_ENABLE
    for (int index = 0 ; index < EFFECT_REVERB_PARM_MAX; index++) {
        struct eff_parm  *mic_eff = &eff_mode[index];
        //low
        mic_eff->tunning_eq_parm.seg[0].filt_type = EQ_IIR_TYPE_BAND_PASS;
        mic_eff->tunning_eq_parm.seg[0].center_freq = 100;
        mic_eff->tunning_eq_parm.seg[0].q = 0.7f;
        mic_eff->tunning_eq_parm.seg[0].min_gain = -12;
        mic_eff->tunning_eq_parm.seg[0].max_gain = 12;
        //mid
        mic_eff->tunning_eq_parm.seg[1].filt_type = EQ_IIR_TYPE_BAND_PASS;
        mic_eff->tunning_eq_parm.seg[1].center_freq = 1000;
        mic_eff->tunning_eq_parm.seg[1].q = 0.7f;
        mic_eff->tunning_eq_parm.seg[1].min_gain = -12;
        mic_eff->tunning_eq_parm.seg[1].max_gain = 12;
        //high
        mic_eff->tunning_eq_parm.seg[2].filt_type = EQ_IIR_TYPE_BAND_PASS;
        mic_eff->tunning_eq_parm.seg[2].center_freq = 12000;
        mic_eff->tunning_eq_parm.seg[2].q = 0.7f;
        mic_eff->tunning_eq_parm.seg[2].min_gain = -12;
        mic_eff->tunning_eq_parm.seg[2].max_gain = 12;
    }
#endif
}
/*
 *混响mic的tunning_eq效果文件配置参数获取
 * */
void mic_tunning_file_analyze_init()
{
#if defined(TCFG_MIC_EFFECT_ENABLE) && TCFG_MIC_EFFECT_ENABLE && BASS_AND_TREBLE_ENABLE
    for (int index = 0 ; index < EFFECT_REVERB_PARM_MAX; index++) {
        struct eff_parm  *mic_eff = &eff_mode[index];
        u16 group_id = EFF_MIC_TUNNING_EQ;
        void *tar_buf = (void *)&mic_eff->tunning_eq_parm;
        u16 tar_len = sizeof(struct tunning_eq_tool);
        eff_file_analyze(eff_mode_seq[index], group_id, tar_buf, tar_len);
#if PARM_DEBUG
        struct tunning_eq_tool *_parm = tar_buf;
        for (int i = 0; i < 3; i++) {
            struct tunning_eq_seg *parm = &_parm->seg[i];
...
void last_whole_wdrc_snap_parm_switch(u8 tar_snap)
{
#if TCFG_DYNAMIC_EQ_ENABLE
    if (!tar_snap) {
        last_whole_wdrc_file_analyze_init();
    } else {
        /* u16 index = 0; */
        u8 calc = mode_add;
        u16 group_id = EFF_MUSIC_LAST_WHOLE_DRC;
